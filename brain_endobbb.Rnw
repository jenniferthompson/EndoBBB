...
fit.nb.models <- function(use.marker,    ## character string; name of biomarker variable to use
  ...
  if(p.nonlin > pcutoff){
    ...
    p.marker <- pc.jt(mod.final, nomark.del.mod.lin)$pvalue
  } else{
    ...
    ## 4. Calculate overall p-value for biomarker
    p.marker <- pc.jt(mod.final, nomark.del.mod.nonlin)$pvalue
  }
...
## -- Function to create design matrix given a model object, X and interacting variables -----------
catvars <- "sevsepsis.day1"
create.designmat <- function(miceobj,              ## mice model object
                             xvar,                 ## string; variable for x axis
                             intvar = NULL,        ## string; variable to interact
                             intvals = NULL,       ## vector; values to set interaction variable to
...
plot.nb.results <- function(nbobj,            ## list, result of fit.nb.models()
  ...
  if(length(grep('final\\.mod', names(nbobj))) == 0){
    ...
    if(is.null(intvar)){
      nb.designmat <- create.designmat(miceobj = nbobj$final.mod, xvar = xvar)
      nb.results <- calc.nbresults(micemod = nbobj$final.mod, designmat = nb.designmat, xvar = xvar)
    } else{
      nb.designmat <- create.designmat(miceobj = nbobj$final.mod,
                                       xvar = xvar,
                                       intvar = intvar,
                                       intvals = intvals)
