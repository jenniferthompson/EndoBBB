...
endo.labels <- c("BDNF", "E-selectin", "PAI-1", "S100B", "UCHL")
...
endo_diffs <- function(endoname, testname = c("rbans", "trailb")){
  ...
  intAdjust_vals <- c(age.intvals, il6.intvals, age.intvals, il6.intvals)
  intVar_vals <- rep(c(rep("age.div10", length(age.intvals)),
                       rep("il6.log10", length(il6.intvals))),
                     2)
  
  ## Things that vary based on biomarker:
  rmsObjName_vals <- c(rep(sprintf("%s.%s.3.mod", endoname, testname),
                           length(age.intvals) + length(il6.intvals)),
                       rep(sprintf("%s.%s.12.mod", endoname, testname),
                           length(age.intvals) + length(il6.intvals)))
  estVals_val <- rep(list(dd$limits[c(1, 3), paste0(endoname, ".log10")]),
                     (length(age.intvals) + length(il6.intvals)) * 2)
  ...
  ## Create data frame of all estimates/CIs, add time point, then keep only
  ## those that are unique (ie, no meaningful interaction)
  diffscores <- pmap_df(.l = marker_args, .f = rms_estimate_with_int) %>%
    mutate(timept = c(rep(3, length(age.intvals) + length(il6.intvals)),
                      rep(12, length(age.intvals) + length(il6.intvals)))) %>%
    group_by(var.main, timept) %>%
    mutate(effect_same = effect == lag(effect),
           keep_this = is.na(effect_same) | !effect_same) %>%
    ungroup() %>%
    filter(keep_this) %>%
    dplyr::select(-keep_this, -effect_same)
...
endo_plotdata <- function(df){
  df <- df %>%
    group_by(var.main, timept) %>%
    ## Get number of unique estimates/CIs;
    ##  if >1, assign 1:number of unique as x values;
    ##  if only 1, x value = 90th %ile of 1:max
    mutate(
      nunique = n(),
      endo_label = paste0(
        endo.labels[match(gsub("\\.log10$", "", var.main), names(endo.labels))],
        ",*\n",
        rndformat(comp.level), " vs. ", rndformat(ref.level)
      ),
      int_label = factor(ifelse(nunique == 1, 1,
                         ifelse(var.int == "age.div10", 2, 3)),
                         levels = 1:3,
                         labels = c("All Patients", "Age", "IL-6*")),
      int_label_faux = factor(ifelse(nunique == 1, 1,
                              ifelse(var.int == "age.div10", 2, 3)),
                              levels = 1:3, labels = c(" ", "  ", "   ")),
      y_label = ifelse(nunique == 1, "",
  ...
  ## Add fake rows - I need more white space for interacting variables
  fakerows <- df %>%
    dplyr::select(endo_label, int_label, int_label_faux, time_label) %>%
    distinct() %>%
    filter(int_label != "All Patients")

  df <- bind_rows(
    df,
    fakerows %>% filter(int_label == "Age") %>% mutate(xval = 0.75, effect = -11),
    fakerows %>% filter(int_label == "IL-6*") %>% mutate(xval = 8.75, effect = -11),
    fakerows %>% mutate(xval = 4.75, effect = -11)
  )
  
  df_intlabels <- df %>%
    dplyr::select(endo_label, int_label, int_label_faux, time_label) %>%
    distinct() %>%
    mutate(xval = ifelse(int_label == "All Patients", 1,
                  ifelse(int_label == "Age", 4.5, 8.5)),
           effect = -11)
  
  return(list("diff_df" = df, "intlevels_df" = df_intlabels))
}
...
plot_ltci_diffs <- function(diffdata_list, testname, difflims, diffbreaks, intx){
  diffdf <- diffdata_list$diff_df
  intdf <- diffdata_list$intlevels_df
  
  ggplot(data = diffdf, aes(x = xval, y = effect, group = effect)) +
  geom_hline(yintercept = 0, linetype = "solid", colour = "grey75", size = 1, alpha = 0.6) +
  facet_grid(endo_label + int_label_faux ~ time_label,
             scales = "free_y", space = "free_y", switch = "y") +
  geom_pointrange(aes(ymin = lowercl, ymax = uppercl), colour = "#003D79", size = 0.5) +
  geom_text(aes(label = int_label, x = xval), data = intdf,
            y = intx, size = 3, fontface = "bold", hjust = 0, colour = "#003D79") +
  geom_text(aes(label = y_label),
            y = intx, size = 2.5, hjust = 0, colour = "#003D79") +
  ...
  theme(axis.title.x = element_text(face = "bold"),
        ...
        strip.text.y = element_text(face = "bold", angle = 180, size = 10,
                                    hjust = 0, vjust = 0.5),
...
rbans_diffplot <- plot_ltci_diffs(
  rbans_diffs,
...
trailb_diffplot <- plot_ltci_diffs(
  trailb_diffs,
  testname = "Trails B",
  difflims = c(-6, 4),
...
pdf(file = "brain_endobbb_rbansdiffs.pdf", width = 8, height = 10)
rbans_diffplot
dev.off()

pdf(file = "brain_endobbb_trailbdiffs.pdf", width = 8, height = 10)
trailb_diffplot
dev.off()
