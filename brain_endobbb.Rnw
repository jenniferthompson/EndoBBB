...
## -- Calculate PEs for differences in RBANS, Trails B for each marker, --------
## -- adjusting to age/IL-6 if needed ------------------------------------------
...
plot_ltci_diffs <- function(diffdata, testname, difflims, diffbreaks){
  ggplot(data = diffdata, aes(x = yval_f, y = effect)) +
  geom_hline(yintercept = 0, linetype = "solid", colour = "grey75", size = 1, alpha = 0.6) +
  facet_grid(time_label ~ int_facet) +
  geom_pointrange(aes(ymin = lowercl, ymax = uppercl), colour = "#003D79", size = 0.5) +
  # scale_x_discrete(name = "") +
  scale_y_continuous(name = paste("\nAdjusted Difference in", testname, "Scores"),
                     limits = difflims, breaks = diffbreaks) +
  coord_flip() +
...
rbans_diffplot_il6 <- plot_ltci_diffs(
  rbans_diffs %>% filter(var.int == "il6.log10"),
  testname = "RBANS Global",
  difflims = c(-9, 4),
  diffbreaks = seq(-9, 3, 3)
)

trailb_diffplot_il6 <- plot_ltci_diffs(
  trailb_diffs %>% filter(var.int == "il6.log10"),
  testname = "Trails B",
  difflims = c(-6, 4),
  diffbreaks = seq(-4, 4, 2)
)

rbans_diffplot_age <- plot_ltci_diffs(
  rbans_diffs %>% filter(var.int == "age.div10"),
  testname = "RBANS Global",
  difflims = c(-6, 4),
  diffbreaks = seq(-6, 3, 3)
)

trailb_diffplot_age <- plot_ltci_diffs(
