...
latex(summaryM(adl.totscore.3 + cohort.noadl.3 + adl.totscore.12 + cohort.noadl.12 + faq.totscore.3 +
                 cohort.nofaq.3 + faq.totscore.12 + cohort.nofaq.12 ~ cohort,
...
fit.reg.models <- function(use.marker,           ## character string; name of biomarker variable
                           ...
                           imp.obj = NULL,       ## aregImpute object to use for ols() if desired
  ...
  } else if(modtype == 'ols' & is.null(imp.obj)){
    modfit.text = "ols(formula = %s, data = usedata)"
  } else if(modtype == 'ols' & !is.null(imp.obj)){
    use.xtrans <- imp.obj
    modfit.text = 'fit.mult.impute(formula = %s, fitter = ols, xtrans = use.xtrans, data = usedata)'
  }
  ...
  if(is.null(p.nonlin) || p.nonlin > pcutoff){
    ...
    if(p.int > pcutoff){
      ...
      p.int.age <- lrpval(mod.lin, mod.noageint)

      if(has.il6){
        mod.noil6int <- eval(parse(text = sprintf(modfit.text, marker.noil6int.formula)))
        p.int.il6 <- lrpval(mod.lin, mod.noil6int)
      }
    }
  } else{
    if(modtype == 'zeroinfl'){
    ...
    if(p.int > 0.2){
      ...
      mod.noageint <- eval(parse(text = sprintf(modfit.text, marker.noageint.formula)))
      p.int.age <- lrpval(mod.full, mod.noageint)

      if(has.il6){
        mod.noil6int <- eval(parse(text = sprintf(modfit.text, marker.noil6int.formula)))
        p.int.il6 <- lrpval(mod.full, mod.noil6int)
      }
    }
  }
  ...
  if(has.il6){
    ...
    p.il6 <- lrpval(mod.final, noil6.mod)
  }
