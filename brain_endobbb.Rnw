...
fit.nb.models <- function(use.marker,     ## character string; name of biomarker variable to use
                          ...
                          nomark.list){   ## list of models without biomarker, named "lin", "nonlin"
...
## -- Function to fit zero-inflated NB model, determine whether 1) nonlinear and 2) interaction
## --  terms are needed, and return final model object, p-values for nonlinearity and interactions,
## --  and final p for biomarker
fit.zeroinfl.models <- function(use.marker,   ## character string; name of biomarker variable
                                formula.full, ## formula for full model formula
  ...
  marker.full.formula <- gsub('markervar', use.marker, formula.full)
  mod.full <-
    eval(parse(text = sprintf("zeroinfl(formula = %s, data = usedata, dist = 'negbin')",
                              marker.full.formula)))

  ## Step 3: Test for whether nonlinear terms are needed
  ## 3a. Fit model with all linear terms
  marker.lin.formula <- gsub(', *[0-9]+\\)', '', gsub('rcs\\(', '', marker.full.formula))
  mod.lin <- eval(parse(text = sprintf("zeroinfl(formula = %s, data = usedata, dist = 'negbin')",
                                       marker.lin.formula)))
  ...
  if(p.nonlin > pcutoff){
    ...
    marker.noint.formula <- create.zeroinfl.form(count.form = form.split.count,
                                                 ...
                                                 regex.sub = ':')
    mod.noint <-
      eval(parse(text = sprintf("zeroinfl(formula = %s, data = usedata)", marker.noint.formula)))
...
zinf.response <- function(modobj, design.zinf){
...
zinf.plotdata <- function(modobj, xvar, bootdata){
  ...
  formula.final <- as.character(bdnf.adl.3$final.mod$call)[2]
  allmods <- lapply(bootdata, FUN = function(dataset){
    zeroinfl(formula = as.formula(formula.final),
             dist = 'negbin',
             data = dataset) })
  
  pred.all <- lapply(design.all, FUN = function(dmat){
    do.call(cbind,
            lapply(allmods, FUN = function(mod){
              zinf.response(mod, dmat)
            }))
  })
...
# ## -- Function to, for each row in design matrix, calculate predicted value & SE from pooled results
# calc.nbresults <- function(micemod,        ## mice model object (unpooled)
#                            designmat,      ## matrix of covariate values; ncols = coefs in micemod
#                            xvar,           ## character string: name of variable to put on X axis
#                            intvar = NULL){ ## character string: name of interacting variable  
# 
#   if(ncol(designmat) != length(micemod$analyses[[1]]$coefficients)){
#     stop("Number of columns in design matrix does not match number of coefficients in mice object")
...
#   } else if(!is.null(intvar) &
#             length(grep(':', names(nbobj$final.mod$analyses[[1]]$coefficients))) == 0){
#     stop('Interacting variable is specified, but model does not include interaction terms')
#   } else if(!is.null(intvar) & is.null(intvals)){
#     stop('Interacting variable is specified, but no interacting values are specified')
#   } else{
#     if(is.null(intvar)){
#       nb.designmat <- create.designmat.mice(miceobj = nbobj$final.mod, xvar = xvar)
#       nb.results <- calc.nbresults(micemod = nbobj$final.mod, designmat = nb.designmat, xvar = xvar)
#     } else{
#       nb.designmat <- create.designmat.mice(miceobj = nbobj$final.mod,
#                                             xvar = xvar,
#                                             intvar = intvar,
#                                             intvals = intvals)
#       
#       nb.results <- calc.nbresults(micemod = nbobj$final.mod,
#                                    designmat = nb.designmat,
#                                    xvar = xvar,
#                                    intvar = intvar)
#       
...
#                             xstring, formatp(nbobj$p.marker), formatp(nbobj$p.nonlin), formatp(nbobj$p.int))))
#     } else{
#       return(nb.plot +
...
and both systemic inflammation (log10(IL-6)) on study day 1 and age at study enrollment. If either
the global p-value for nonlinearity and/or the global p-value for interactions were clearly
...
to improve model fit and reduce the influence of extremely high values. All p-values are calculated
using the \Sexpr{use.method} method.
...
%'   fit.nb.models(use.marker = marker,
%'                 mice.obj = 'mice.del',
%'                 formula.full = del.full.formula,
%'                 nomark.list = list('nonlin' = nomark.del.mod.nonlin,
%'                                    'lin' = nomark.del.mod.lin))
%' }
...
%' # pool.compare(del.s100b.noint, nomark.del.mod.nonlin, method = use.method, data = mice.del)$pvalue
%' # s100b.del$p.marker
%' 
%' ## -- Create graphics for delirium models ----------------------------------------------------------
%' nb.figures <- function(miceobj, xvar, xstring, ystring){
%'   if(miceobj$p.int > 0.20){
%'     return(list('fig' = plot.nb.results(nbobj = miceobj,
%'                                         xvar = xvar,
%'                                         xstring = xstring,
%'                                         ystring = ystring),
%'                 'height' =  4,
...
%'   } else{
%'     fig1 <- plot.nb.results(nbobj = miceobj, xvar = xvar,
%'                             intvar = 'age.enroll', intvals = intvals.age,
%'                             intstring = 'Age', intp = miceobj$p.int.age,
%'                             xstring = xstring, ystring = ystring)
%'     fig2 <- plot.nb.results(nbobj = miceobj, xvar = xvar,
%'                             intvar = 'il6.log10', intvals = intvals.il6,
%'                             intstring = 'Log10(IL-6)', intp = miceobj$p.int.il6,
%'                             xstring = xstring, ystring = ystring)
%'     
...
%' 
%' bdnf.del.fig <-
%'   nb.figures(miceobj = bdnf.del, xvar = 'bdnf.log10', xstring = 'BDNF', ystring = 'Days of Delirium')
%' esel.del.fig <-
%'   nb.figures(miceobj = esel.del, xvar = 'esel.log10', xstring = 'E-Selectin', ystring = 'Days of Delirium')
%' pai1.del.fig <-
%'   nb.figures(miceobj = pai1.del, xvar = 'pai1.log10', xstring = 'PAI-1', ystring = 'Days of Delirium')
%' s100b.del.fig <-
%'   nb.figures(miceobj = s100b.del, xvar = 's100b.log10', xstring = 'S100B', ystring = 'Days of Delirium')
%' uchl.del.fig <-
%'   nb.figures(miceobj = uchl.del, xvar = 'uchl.log10', xstring = 'UCHL', ystring = 'Days of Delirium')
%' 
%' pdf(file = 'bdnf_del_results.pdf', width = 8, height = bdnf.del.fig$height)
%' multiplot(bdnf.del.fig$fig, layout = bdnf.del.fig$layoutmat)
%' dev.off()
%' 
%' pdf(file = 'esel_del_results.pdf', width = 8, height = esel.del.fig$height)
%' multiplot(esel.del.fig$fig, layout = esel.del.fig$layoutmat)
%' dev.off()
%' 
...
%' multiplot(s100b.del.fig$fig, layout = s100b.del.fig$layoutmat)
%' dev.off()
%' 
%' pdf(file = 'uchl_del_results.pdf', width = 8, height = uchl.del.fig$height)
%' multiplot(uchl.del.fig$fig, layout = uchl.del.fig$layoutmat)
%' dev.off()
%' 
%' @
%' 
...
%' The overall association between log10(PAI-1) and delirium duration is nonsignificant. It is
%' possible that there is a mild association modified by age at enrollment, such that among younger
%' patients, higher log10(PAI-1) is predictive of fewer delirium days, but this association gets
%' weaker as patient age increases.
%' 
%' Since the overall p-value for interaction met our cutoff of \Sexpr{pcutoff}, the final model also
%' includes an interaction with log10(IL-6); however, as seen in the figure, any association between
%' log10(PAI-1) and delirium duration does not seem to be substantially modified by systemic
%' inflammation.
%' 
%' \begin{figure}[!h]
...
%' UCHL at baseline does show a meaningful association with days of delirium. This association is not
%' significantly nonlinear, so we can say that higher log10(UCHL) at study enrollment is nearly
%' significantly associated with fewer days of delirium. This association is not modified by age or
...
\item Katz ADL at enrollment (divided by 10)
...
We allowed all continuous variables to have a nonlinear association with delirium duration using
restricted cubic splines, with the exception of ADL at baseline, which has a distribution that is too skewed to allow this. We allowed for potential interactions between endothelial/BBB markers
and both systemic inflammation (log10(IL-6)) on study day 1 and age at study enrollment. If either
the global p-value for nonlinearity and/or the global p-value for interactions were clearly
nonsignificant (p $>$ \Sexpr{format(pcutoff, nsmall = 2)}), those terms are removed from the
final model for parsimony. All biomarkers are transformed using the log10 transformation in order
...
adl.righthand <- "rcs(charlson.score, 3) + rcs(edu, 3) + rcs(iqcode.score.e, 3) + rcs(stroke.risk, 3) + adl.e + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(il6.log10, 3) + rcs(markervar, 3) + rcs(age.div10, 3):rcs(markervar, 3) + rcs(il6.log10, 3):rcs(markervar, 3) | rcs(age.div10, 3) + rcs(charlson.score, 3) + adl.e + rcs(mean.modsofa.icu, 3)"
...
fit.adl.models.3 <- function(marker){
  fit.zeroinfl.models(use.marker = marker,
                      formula.full = adl.full.formula.3,
                      dataset = endo.data)
}
...
# ## Test BDNF by hand to make sure
# endo.data.adl.bdnf.3 <-
...
#                                        'bdnf.log10')])) == 0,]
# bdnf.org <- zeroinfl(adl.totscore.3 ~ rcs(charlson.score, 3) + rcs(edu, 3) +
#                         rcs(iqcode.score.e, 3) + rcs(stroke.risk, 3) + adl.e +
#                         rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) +
#                         rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(il6.log10, 3) +
#                         rcs(bdnf.log10, 3) + rcs(age.div10, 3):rcs(bdnf.log10, 3) +
#                         rcs(il6.log10, 3):rcs(bdnf.log10, 3) |
#                        rcs(age.div10, 3) + rcs(charlson.score, 3) + adl.e +
#                         rcs(mean.modsofa.icu, 3),
#                      dist = 'negbin',
#                      data = endo.data.adl.bdnf.3)
# 
# ## Linear terms
# bdnf.lin <- zeroinfl(adl.totscore.3 ~ charlson.score + edu + iqcode.score.e + stroke.risk + adl.e +
#                         mean.modsofa.icu + icudays.sevseptic.s + del.s.imp + coma.s.imp +
#                         age.div10 + il6.log10 + bdnf.log10 + age.div10:bdnf.log10 +
#                         il6.log10:bdnf.log10 |
#                        age.div10 + charlson.score + adl.e + mean.modsofa.icu,
#                      dist = 'negbin',
#                      data = endo.data.adl.bdnf.3)
# 
# lmtest::lrtest(bdnf.org, bdnf.lin)
# bdnf.adl.3$p.nonlin
# 
# ## Nonlinear terms needed; test for interactions
# bdnf.noint <- zeroinfl(adl.totscore.3 ~ rcs(charlson.score, 3) + rcs(edu, 3) +
#                         rcs(iqcode.score.e, 3) + rcs(stroke.risk, 3) + adl.e +
#                         rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) +
#                         rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(il6.log10, 3) +
#                         rcs(bdnf.log10, 3) |
...
#                      dist = 'negbin',
#                      data = endo.data.adl.bdnf.3)
# lmtest::lrtest(bdnf.org, bdnf.noint)
# bdnf.adl.3$p.int
# 
# ## Interaction terms needed; test for age and IL-6 separately
# bdnf.noage <- zeroinfl(adl.totscore.3 ~ rcs(charlson.score, 3) + rcs(edu, 3) +
#                         rcs(iqcode.score.e, 3) + rcs(stroke.risk, 3) + adl.e +
#                         rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) +
#                         rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(il6.log10, 3) +
#                         rcs(bdnf.log10, 3) + rcs(il6.log10, 3):rcs(bdnf.log10, 3) |
#                        rcs(age.div10, 3) + rcs(charlson.score, 3) + adl.e +
#                         rcs(mean.modsofa.icu, 3),
#                      dist = 'negbin',
#                      data = endo.data.adl.bdnf.3)
# lmtest::lrtest(bdnf.org, bdnf.noage)
# bdnf.adl.3$p.int.age
# 
# bdnf.noil6 <- zeroinfl(adl.totscore.3 ~ rcs(charlson.score, 3) + rcs(edu, 3) +
#                         rcs(iqcode.score.e, 3) + rcs(stroke.risk, 3) + adl.e +
#                         rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) +
#                         rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(il6.log10, 3) +
#                         rcs(bdnf.log10, 3) + rcs(age.div10, 3):rcs(bdnf.log10, 3) |
...
#                      dist = 'negbin',
#                      data = endo.data.adl.bdnf.3)
# lmtest::lrtest(bdnf.org, bdnf.noil6)
# bdnf.adl.3$p.int.il6
# 
# ## Test for biomarker
# bdnf.nomark <- zeroinfl(adl.totscore.3 ~ rcs(charlson.score, 3) + rcs(edu, 3) +
#                         rcs(iqcode.score.e, 3) + rcs(stroke.risk, 3) + adl.e +
#                         rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) +
#                         rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(il6.log10, 3) |
#                        rcs(age.div10, 3) + rcs(charlson.score, 3) + adl.e +
...
#                      dist = 'negbin',
#                      data = endo.data.adl.bdnf.3)
# lmtest::lrtest(bdnf.org, bdnf.nomark)
# bdnf.adl.3$p.marker
...
fit.adl.models.12 <- function(marker){
  fit.zeroinfl.models(use.marker = marker,
                      formula.full = adl.full.formula.12,
                      dataset = endo.data)
}
