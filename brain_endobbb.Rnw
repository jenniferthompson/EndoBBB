...
marker.data <- bind_rows(lapply(endo.names, FUN = function(x){
  ...
  dplyr::select(id, marker, mean.pgml) %>%
...
marker.hist.data.org <- endo.data %>%
  dplyr::select(il6.day1, bdnf.day1, esel.day1, pai1.day1, s100b.day1, uchl.day1) %>%
  gather(key = marker, value = markerval) %>%
  mutate(marker = toupper(gsub('\\.day1$', '', marker)))

marker.hist.data.log10 <- endo.data %>%
  dplyr::select(il6.log10, bdnf.log10, esel.log10, pai1.log10, s100b.log10, uchl.log10) %>%
  gather(key = marker, value = markerval) %>%
...
## -- Function: Combine rms model results with, w/o adjustment for IL-6 --------
latex_combined_results <- function(l_il6, l_noil6, caption){
  m_il6 <-
    pluck(l_il6, "final.mod") %>% rms_model_results(printFormat = "latex")
  m_noil6 <-
    pluck(l_noil6, "final.mod") %>% rms_model_results(printFormat = "latex")

  df <- left_join(m_il6, m_noil6, by = c("label", "low", "high")) %>%
    filter(
      !(label %in%
          c("", paste0("\\emph{", c("Model L.R.", "$R^{2}$", "n"), "}")))
    ) %>%
    dplyr::select(label, low, high, est.ci.x, pval.x, est.ci.y, pval.y) %>%
    latex(
      file = "",
      caption = caption,
      colheads = c("Variable", "25th", "75th", rep(c("Estimate (CI)", "P"), 2)),
      col.just = c("l", rep("r", 10)),
      cgroup = c("", "Adjusted for IL-6", "Not Adjusted for IL-6"),
      n.cgroup = c(3, 2, 2),
      rowname = NULL,
      where = "!h"
    )
...
mice.vars <- c(
  'bdnf.log10', 'bdnf.prime', 'bdnf.age', 'bdnfp.age', 'bdnf.agep', 'bdnfp.agep',
...
mice.prev <- mice(
  data = endo.data[, gsub("del.s.imp", "ever.del.s.imp", mice.vars)],
  m = nimp,
  seed = 56
)
mice.del <- mice(data = endo.data[,mice.vars], m = nimp, seed = 56)
mice.3 <-
  mice(data = endo.data[endo.data$fu.cohort.3, mice.vars], m = nimp, seed = 56)
mice.12 <- mice(data = endo.data[endo.data$fu.cohort.12, mice.vars], m = nimp, seed = 56)

## -- Create aregImpute object for prevalence model (quicker than making -------
## -- already-written functions play nicely with mice objects) -----------------
areg.prev.form <- as.formula(
  "~ age.div10 + charlson.score + I(edu.div10) + I(iqcode.score.e) + apache.aps + stroke.risk + cvsofa.day1 + sevsepsis.day1 + ever.del.s.imp + coma.s.imp + il6.log10 + bdnf.log10 + esel.log10 + pai1.log10 + s100b.log10 + uchl.log10"
)

set.seed(56)
areg.prev <- aregImpute(areg.prev.form, n.impute = 10, nk = 3, data = endo.data)
...
## Play a sound, because we're done with imputation
if(require("BRRR")){
  BRRR::skrrrahh("snoop")
}
...
## -- FUNCTIONS FOR LOGISTIC MODELS (delirium prevalence) ----------------------

## -- Function to fit logistic regression models, w/ mice or complete case -----
## Tests (in order)
##   1) nonlinear terms
##   2) interaction terms
##   3) age and IL-6 interactions
##   4) biomarker variables
## Yes, this is nearly the same as fit.ols.models() and could be coded more
##  efficiently. We are in get-it-done-without-messing-things-up mode. Sorry.
fit.lrm.models <- function(
  formula.right,     ## original model covariates
  outcome,           ## string; outcome variable
  impobj = NULL,     ## mice or aregImpute imputation object
  dataset = NULL,    ## data set to use
  cohort.var = NULL, ## Indicator variable in dataset for cohort
  ...
  ## Create text for fitting model, depending on whether imputation is used
  if(is.null(impobj)){
    modfit.text <- "lrm(formula = %s, data = usedata)"
  } else if(is.mids(impobj)){
    modfit.text <- "with(impobj, lrm(formula = %s))"
  } else{
    modfit.text <-
      "fit.mult.impute(formula = %s, fitter = lrm, xtrans = impobj, data = usedata)"
  }

  ## Is impobj a) defined and b) of class mids?
  imp.mice <- !is.null(impobj) & is.mids(impobj)
  
  ## If impobj is an aregImpute object, dataset must be provided
  if(!is.null(impobj) & !imp.mice & is.null(dataset)){
    stop("If impobj is created by aregImpute(), dataset must be provided")
  }
  ...
  ## Initialize values
  need.nonlin <- p.nonlin <- need.int <- p.int <- p.int.age <- p.int.il6 <-
    p.il6 <- p.marker <- marker.ci <- marker.cistr <- il6.ci <- il6.cistr <- NULL
  ...
  ## 5: Get IRR/CI for 75th vs 25th percentile for final model
  marker.ci <- exp(summary(final.mod)[paste0(marker, '.log10'), c('Effect', 'Lower 0.95', 'Upper 0.95')])
  names(marker.ci) <- c('pointest', 'lcl', 'ucl')
  marker.cistr <- paste0(rndformat(marker.ci['pointest']), ' (',
                         rndformat(marker.ci['lcl']), ', ',
                         rndformat(marker.ci['ucl']), ')')
  ...
  if(has.il6){
    il6.ci <- exp(summary(final.mod)['il6.log10', c('Effect', 'Lower 0.95', 'Upper 0.95')])
    names(il6.ci) <- c('pointest', 'lcl', 'ucl')
    il6.cistr <- paste0(rndformat(il6.ci['pointest']), ' (',
                        rndformat(il6.ci['lcl']), ', ',
                        rndformat(il6.ci['ucl']), ')')
...
calc.lrm.results <- function(
  lrmobj,          ## string, name of lrm() result
  xvar,            ## string, variable for X axis
  intvars = NULL,  ## string, interacting variable
  intvals = NULL
){ ## list of numeric vectors, values of intvars
  if(!is.null(intvars) & (is.null(intvals) | length(intvars) != length(intvals))){
    stop("Interacting variable(s) specified, but length of interacting values does not match number of interacting variables")
  } else{
    if(is.null(intvars)){
      preddata <- list(
        eval(
          parse(
            text = sprintf(
              'as.data.frame(Predict(%s, %s = NA, fun = plogis))', lrmobj, xvar
            )
          )
        )
      )
      ...
      preddata <- lapply(1:length(intvars), FUN = function(ivar){
        eval(
          parse(
            text = sprintf(
              'as.data.frame(Predict(%s, %s = NA, %s = c(%s), fun = plogis))',
              lrmobj, xvar, intvars[ivar],
              paste(intvals[ivar], collapse = ', ')
            )
          )
        )
...
## -- Create plot using one data set of predicted values -----------------------
plot.lrm.results <- function(
  lrm.preddata,     ## data frame of predicted values
  lrm.list,         ## result of fit.reg.models()
  xvar,             ## string; variable on X axis
  intvar = NULL,    ## string; variable to facet
  intstring = NULL, ## string; text to describe faceting variable
  xstring,          ## string; description of X variable (Log10(xstring))
  ...
  if(!is.null(intvar)){
    lrm.preddata$intvar.f <-
      factor(paste(intstring, rndformat(lrm.preddata[,intvar], 2), sep = ' = '))
  }

  fig <- ggplot(aes_string(x = xvar, y = 'yhat'), data = lrm.preddata) +
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = 'navy') +
    geom_line(colour = 'navy') +
    scale_y_continuous(name = paste("Adjusted", ystring), limits = c(0, 1)) +
    theme_bw() +
    theme(axis.title.x = element_text(size = 9, vjust = 0),
          axis.title.y = element_text(size = 9),
          axis.text = element_text(size = 7)) +
...
## -- Create graphics for logistic models --------------------------------------
lrm.figures <- function(lrmlist,
                        xvar,
                        xstring,
                        ystring){

  lrmlist.got <- get(lrmlist)

  if(lrmlist.got$p.int > 0.20){
    preddata <-
      calc.lrm.results(lrmobj = paste0(lrmlist, '$final.mod'), xvar = xvar)
    return(list('fig' = list(plot.lrm.results(lrm.preddata = preddata[[1]],
                                              lrm.list = lrmlist.got,
                                              xvar = xvar,
                                              xstring = xstring,
                                              ystring = ystring)),
                'height' =  onepanel.height,
    ...
    if(!is.null(lrmlist.got$p.int.age)){
      preddata.age <- calc.lrm.results(lrmobj = paste0(lrmlist, '$final.mod'),
                                       xvar = xvar,
                                       intvars = 'age.div10',
                                       intvals = list(intvals.age / 10))
      fig1 <- plot.lrm.results(lrm.preddata = preddata.age[[1]],
                               lrm.list = lrmlist.got,
                               xvar = xvar,
                               intvar = 'age.div10',
                               intstring = 'Age / 10',
    ...
    if(!is.null(lrmlist.got$p.int.il6)){
      preddata.il6 <- calc.lrm.results(lrmobj = paste0(lrmlist, '$final.mod'),
                                       xvar = xvar,
                                       intvars = 'il6.log10',
                                       intvals = list(intvals.il6))
      fig2 <- plot.lrm.results(lrm.preddata = preddata.il6[[1]],
                               lrm.list = lrmlist.got,
                               xvar = xvar,
                               intvar = 'il6.log10',
                               intstring = 'Log10(IL-6)',
    ...
    if(!is.null(lrmlist.got$p.int.age) & !is.null(lrmlist.got$p.int.il6)){
      fig.list <- list(fig1, fig2)
      fig.height <- twopanel.height
      fig.layout <- matrix(1:2, ncol = 1)
    } else{
...
\section{Endothelial/BBB Injury Markers vs. Delirium Prevalence}

To examine the association between endothelial/BBB injury at study enrollment and delirium prevalence (ever/never experienced delirium) in the 30 days following, we used logistic regression, adjusting for the following covariates:
...
We allowed endothelial biomarkers and IL-6 to have nonlinear associations with delirium prevalence
using restricted cubic splines; all other continuous covariates were forced to be linear to improve model stability and reproducibility.

\textbf{Note:} Delirium prevalence is a highly blunt outcome. When all critically ill patients are included, it has the potential to be highly biased, since patients who die quickly have much less opportunity to experience delirium. Here, our entire cohort comprises survivors of critical illness; this means that the issue of survivorship bias is less problematic, but does mean that interpretation should be done very carefully, drawing conclusions \emph{only} about survivors of critical illness.
...
<<prevmodels>>=
## Formula for delirium prevalence models: all terms linear except for
##  biomarkers, for consistency with delirium duration models
prev.rhs <- "charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 * rcs(marker.log10, 3)"
prev.rhs.il6 <- "charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 * rcs(marker.log10, 3) + rcs(il6.log10, 3) * rcs(marker.log10, 3)"

## -- Fit models for each biomarker --------------------------------------------
## Wrapper for IL-6, non-IL-6 versions, because I can't stand the repetition
prev_noil6 <- function(marker, ...){
  fit.lrm.models(
    formula.right = prev.rhs,
    outcome = "ever.del.s.imp",
    impobj = areg.prev,
    dataset = endo.data,
    marker = marker
  )
...
prev_il6 <- function(marker, ...){
  fit.lrm.models(
    formula.right = prev.rhs.il6,
    outcome = "ever.del.s.imp",
    impobj = areg.prev,
    dataset = endo.data,
    marker = marker
...
bdnf.prev <- prev_noil6("bdnf")
bdnf.prev.il6 <- prev_il6("bdnf")
esel.prev <- prev_noil6("esel")
esel.prev.il6 <- prev_il6("esel")
pai1.prev <- prev_noil6("pai1")
pai1.prev.il6 <- prev_il6("pai1")
s100b.prev <- prev_noil6("s100b")
s100b.prev.il6 <- prev_il6("s100b")
uchl.prev <- prev_noil6("uchl")
uchl.prev.il6 <- prev_il6("uchl")

## -- Create figures for delirium prevalence -----------------------------------
bdnf.prev.fig <- lrm.figures(
  lrmlist = 'bdnf.prev',
  xvar = 'bdnf.log10', xstring = 'BDNF',
  ystring = "Probability of\nExperiencing Delirium"
)
esel.prev.fig <- lrm.figures(
  lrmlist = 'esel.prev',
  xvar = 'esel.log10', xstring = 'E-selectin',
  ystring = "Probability of\nExperiencing Delirium"
)
pai1.prev.fig <- lrm.figures(
  lrmlist = 'pai1.prev',
  xvar = 'pai1.log10', xstring = 'PAI-1',
  ystring = "Probability of\nExperiencing Delirium"
)
s100b.prev.fig <- lrm.figures(
  lrmlist = 's100b.prev',
  xvar = 's100b.log10', xstring = 'S100B',
  ystring = "Probability of\nExperiencing Delirium"
)
uchl.prev.fig <- lrm.figures(
  lrmlist = 'uchl.prev',
  xvar = 'uchl.log10', xstring = 'UCHL',
  ystring = "Probability of\nExperiencing Delirium"
)

bdnf.prev.il6.fig <- lrm.figures(
  lrmlist = 'bdnf.prev.il6',
  xvar = 'bdnf.log10', xstring = 'BDNF',
  ystring = "Probability of\nExperiencing Delirium"
)
esel.prev.il6.fig <- lrm.figures(
  lrmlist = 'esel.prev.il6',
  xvar = 'esel.log10', xstring = 'E-selectin',
  ystring = "Probability of\nExperiencing Delirium"
)
pai1.prev.il6.fig <- lrm.figures(
  lrmlist = 'pai1.prev.il6',
  xvar = 'pai1.log10', xstring = 'PAI-1',
  ystring = "Probability of\nExperiencing Delirium"
)
s100b.prev.il6.fig <- lrm.figures(
  lrmlist = 's100b.prev.il6',
  xvar = 's100b.log10', xstring = 'S100B',
  ystring = "Probability of\nExperiencing Delirium"
)
uchl.prev.il6.fig <- lrm.figures(
  lrmlist = 'uchl.prev.il6',
  xvar = 'uchl.log10', xstring = 'UCHL',
  ystring = "Probability of\nExperiencing Delirium"
)

## Save all these as PDFs
marker_pdf <- function(figobj, fname){
  pdf(file = fname, width = 8, height = figobj$height)
  multiplot(plotlist = figobj$fig, layout = figobj$layoutmat)
  dev.off()
}

walk2(
  .x = list(
    bdnf.prev.fig, esel.prev.fig, pai1.prev.fig, s100b.prev.fig, uchl.prev.fig,
    bdnf.prev.il6.fig, esel.prev.il6.fig, pai1.prev.il6.fig, s100b.prev.il6.fig,
    uchl.prev.il6.fig
  ),
  .y = c(
    paste0(endo.names, "_prev_results.pdf"),
    paste0(endo.names, "_prev_il6_results.pdf")
  ),
  .f = marker_pdf
)

## -- Publication version for UCHL: tweak X axis, add p-value ------------------
pdf(file = 'uchl_prev_il6_results_pub.pdf', width = 6, height = 4)
uchl.prev.il6.fig$fig[[1]] +
  scale_x_continuous(name = 'Log10(UCHL) Plasma Concentration') +
  annotate(geom = 'text', x = 4.25, y = 0.01, hjust = 1, vjust = 0, size = 3,
           label = paste('P, overall association:',
                         format(round(uchl.prev.il6$p.marker, 2), nsmall = 2)))
dev.off()

@

\clearpage
\subsection{BDNF vs. Delirium Prevalence}
We see no association between log10(BDNF) at baseline and with delirium duration
while in the hospital, whether adjusting for systemic inflammation or not.

<<printbdnfprev, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(bdnf.prev.il6, 'BDNF'),
                       create.pval.table(bdnf.prev, 'BDNF')),
                 caption = 'BDNF vs Delirium Prevalence',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

\clearpage
\subsubsection{BDNF vs. Delirium Prevalence, Adjusting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{bdnf_prev_il6_results.pdf}}
\caption{Log10(BDNF) vs. Delirium Prevalence, Adjusting for Log10(IL-6)}\label{fig:bdnfprevil6}
\end{figure}

\subsubsection{BDNF vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{bdnf_prev_results.pdf}}
\caption{Log10(BDNF) vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}\label{fig:bdnfprevnoil6}
\end{figure}

\clearpage
\subsection{E-Selectin vs. Delirium Prevalence}
We see no association between log(E-selectin) at baseline and with delirium prevalence while in the
hospital, whether adjusting for systemic inflammation or not.

<<printeselprev, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(esel.prev.il6, 'E-selectin'),
                       create.pval.table(esel.prev, 'E-selectin')),
                 caption = 'E-selectin vs Delirium Prevalence',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

\clearpage
\subsubsection{E-Selectin vs. Delirium Prevalence, Adjusting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{esel_prev_il6_results.pdf}}
\caption{Log10(E-selectin) vs. Delirium Prevalence, Adjusting for Log10(IL-6)}\label{fig:eselprevil6}
\end{figure}

\subsubsection{E-Selectin vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{esel_prev_results.pdf}}
\caption{Log10(E-selectin) vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}\label{fig:eselprevnoil6}
\end{figure}

\clearpage
\subsection{PAI-1 vs. Delirium Prevalence}
We see no significant association between log(PAI-1) at baseline and delirium prevalence while
in the hospital, regardless of adjustment for systemic inflammation.

<<printpai1prev, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(pai1.prev.il6, 'PAI-1'),
                       create.pval.table(pai1.prev, 'PAI-1')),
                 caption = 'PAI-1 vs Delirium Prevalence',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

\clearpage
\subsubsection{PAI-1 vs. Delirium Prevalence, Adjusting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{pai1_prev_il6_results.pdf}}
\caption{Log10(PAI-1) vs. Delirium Prevalence, Adjusting for Log10(IL-6)}\label{fig:pai1prev}
\end{figure}

\subsubsection{PAI-1 vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{pai1_prev_results.pdf}}
\caption{Log10(PAI-1) vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}\label{fig:pai1prevnoil6}
\end{figure}

\clearpage
\subsection{S100B vs. Delirium Prevalence}
When not accounting for systemic inflammation, we see no significant association between
log10(S100B) and delirium prevalence. When adjusting for systemic inflammation, we see a potential interaction between log10(IL-6) and log10(S100B), such that there may be a stronger relationship between S100B and delirium prevalence at higher levels of inflammation; however, this relationship is not strong enough to reach overall significance.

<<prints100bprev, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(s100b.prev.il6, 'S100B'),
                       create.pval.table(s100b.prev, 'S100B')),
                 caption = 'S100B vs Days of Delirium',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\clearpage
\subsubsection{S100B vs. Delirium Prevalence, Adjusting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{s100b_prev_il6_results.pdf}}
\caption{Log10(S100B) vs. Delirium Prevalence, Adjusting for Log10(IL-6)}\label{fig:s100bprev}
\end{figure}

\subsubsection{S100B vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{s100b_prev_results.pdf}}
\caption{Log10(S100B) vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}\label{fig:s100bprevnoil6}
\end{figure}

\clearpage
\subsection{UCHL vs. Delirium Prevlanece}
Higher UCHL at baseline does show an association with lower likelihood of delirium during critical illness after accounting for systemic inflammation; this association is weaker and may be modified by age if systemic inflammation is ignored.

<<printuchlprev, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(uchl.prev.il6, 'UCHL'),
                       create.pval.table(uchl.prev, 'UCHL')),
                 caption = 'UCHL vs Delirium Prevalence',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

\clearpage
\subsubsection{UCHL vs. Delirium Prevalence, Adjusting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{uchl_prev_il6_results.pdf}}
\caption{Log10(UCHL) vs. Delirium Prevalence, Adjusting for Log10(IL-6)}\label{fig:uchlprev}
\end{figure}

\subsubsection{UCHL vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}
\begin{figure}[!h]
{\centering\includegraphics{uchl_prev_results.pdf}}
\caption{Log10(UCHL) vs. Delirium Prevalence, Not Accounting for Log10(IL-6)}\label{fig:uchlprevnoil6}
\end{figure}

\clearpage
\section{Primary Analyses: Without Delirium Duration}
\subsection{Endothelial/BBB Injury Markers vs. Katz ADL}
...
func.covars <- c('charlson.score', 'iqcode.score.e', 'edu.div10', 'disabled.e', 'stroke.risk',
                 'mean.modsofa.icu', #'icudays.sevseptic.s', 'del.s.imp',
                 'coma.s.imp',
...
at 3 months after discharge. This association is modified by age (with the interaction for IL-6 left in the model as well). At 12 months, the association has weakened substantially.
...
We see a significant overall association, modified by age when not adjusting for log10(IL-6), between log10(E-selectin) at baseline and Katz ADL scores 3 months after discharge. This association is no longer present by 12-month followup.
...
there is a significant association at 12 months when not adjusting for log10(IL-6).
...
\clearpage
\subsection{Endothelial/BBB Injury Markers vs. Functional Activities Questionnaire}
...
\clearpage
\subsection{Endothelial/BBB Injury Markers vs. RBANS Global Score}
...
cog.righthand <- "rcs(charlson.score, 3) + rcs(edu.div10, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3)*rcs(marker.log10, 3) + rcs(il6.log10, 3)*rcs(marker.log10, 3)"
...
# ## Fit full model
# s100b.full.rbans.3 <- fit.mult.impute(rbans.global.score.3 ~ rcs(charlson.score, 3) + rcs(edu.div10, 3) +
...
#                                       fitter = ols,
#                                       xtrans = areg.3,
#                                       data = endo.data[endo.data$fu.cohort.3,])
...
cog.righthand.noil6 <- "rcs(charlson.score, 3) + rcs(edu.div10, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3)*rcs(marker.log10, 3)"
...
\clearpage
<<printbdnfmodresults, results = "asis">>=
latex_combined_results(
  bdnf.rbans.3, bdnf.rbans.noil6.3, "BDNF vs RBANS, 3 Months"
)

latex_combined_results(
  bdnf.rbans.12, bdnf.rbans.noil6.12, "BDNF vs RBANS, 12 Months"
)
...
\clearpage
<<printeselmodresults, results = "asis">>=
latex_combined_results(
  esel.rbans.3, esel.rbans.noil6.3, "E-selectin vs RBANS, 3 Months"
)

latex_combined_results(
  esel.rbans.12, esel.rbans.noil6.12, "E-selectin vs RBANS, 12 Months"
)
...
We see no significant overall association between log10(PAI-1) at baseline and RBANS scores at
either 3- or 12-month followup.
...
\clearpage
<<printpai1modresults, results = "asis">>=
latex_combined_results(
  pai1.rbans.3, pai1.rbans.noil6.3, "PAI-1 vs RBANS, 3 Months"
)

latex_combined_results(
  pai1.rbans.12, pai1.rbans.noil6.12, "PAI-1 vs RBANS, 12 Months"
)
...
We see a significant overall association between log10(S100B) at baseline and
RBANS scores at both 3- and 12-month followup.
...
\clearpage
<<prints100bmodresults, results = "asis">>=
latex_combined_results(
  s100b.rbans.3, s100b.rbans.noil6.3, "S100B vs RBANS, 3 Months"
)

latex_combined_results(
  s100b.rbans.12, s100b.rbans.noil6.12, "S100B vs RBANS, 12 Months"
)
...
\clearpage
<<printuchlmodresults, results = "asis">>=
latex_combined_results(
  uchl.rbans.3, uchl.rbans.noil6.3, "UCHL vs RBANS, 3 Months"
)

latex_combined_results(
  uchl.rbans.12, uchl.rbans.noil6.12, "UCHL vs RBANS, 12 Months"
)
...
\clearpage
\subsection{Endothelial/BBB Injury Markers vs. Trails B T-Score}
...
We see a marginally significant association between log10(PAI-1) and Trails B t-scores at 12 months, but no association at 3 months.
...
<<publication_forest>>=
## -- Calculate PE, CIs for differences in/IRRs for long-term outcomes for -----
## -- each marker, adjusting to age/IL-6 if needed -----------------------------

## -- Labels for each biomarker ------------------------------------------------
endo.labels <- c("BDNF", "E-selectin", "PAI-1", "S100B", "UCHL1")
names(endo.labels) <- endo.names

endo_flabels <- map_chr(
  endo.names,
  ~ sprintf(
    "%s*,\n%s vs %s",
    endo.labels[match(., names(endo.labels))],
    rndformat(dd$limits["High:effect",paste0(., ".log10")]),
    rndformat(dd$limits["Low:effect",paste0(., ".log10")])
  )
)

## Which quantiles do we want to plot for age, IL-6?
plot_quantiles <- c(0.25, 0.50, 0.75, 0.90)
age_plotvals <-
  quantile(endo.data[,'age.div10'], probs = plot_quantiles, na.rm = TRUE)
il6_plotvals <-
  quantile(endo.data[,'il6.log10'], probs = plot_quantiles, na.rm = TRUE)
## Use il6p_plotvals in disability plots; go ahead and calculate with the others
il6_knots <- rcspline.eval(endo.data$il6.log10, nk = 3, knots.only = TRUE)
il6p_plotvals <- map_dbl(
  il6_plotvals,
  ~ as.numeric(rcspline.eval(., knots = il6_knots))
)

## -- LTCI outcomes ------------------------------------------------------------
## rms_estimate_with_int doesn't like the $
bdnf.rbans.3.mod <- bdnf.rbans.3$final.mod
bdnf.rbans.12.mod <- bdnf.rbans.12$final.mod
esel.rbans.3.mod <- esel.rbans.3$final.mod
esel.rbans.12.mod <- esel.rbans.12$final.mod
pai1.rbans.3.mod <- pai1.rbans.3$final.mod
pai1.rbans.12.mod <- pai1.rbans.12$final.mod
s100b.rbans.3.mod <- s100b.rbans.3$final.mod
s100b.rbans.12.mod <- s100b.rbans.12$final.mod
uchl.rbans.3.mod <- uchl.rbans.3$final.mod
uchl.rbans.12.mod <- uchl.rbans.12$final.mod

bdnf.trailb.3.mod <- bdnf.trailb.3$final.mod
bdnf.trailb.12.mod <- bdnf.trailb.12$final.mod
esel.trailb.3.mod <- esel.trailb.3$final.mod
esel.trailb.12.mod <- esel.trailb.12$final.mod
pai1.trailb.3.mod <- pai1.trailb.3$final.mod
pai1.trailb.12.mod <- pai1.trailb.12$final.mod
s100b.trailb.3.mod <- s100b.trailb.3$final.mod
s100b.trailb.12.mod <- s100b.trailb.12$final.mod
uchl.trailb.3.mod <- uchl.trailb.3$final.mod
uchl.trailb.12.mod <- uchl.trailb.12$final.mod

## Wrapper function for rms_estimate_with_int which creates argument list
## based on a biomarker prefix (endo.names) and calls pmap_df() to get adjusted
## differences
endo_diffs <- function(endoname, testname = c("rbans", "trailb")){
  ## Things that will be the same for every biomarker:
  intAdjust_vals <- c(age_plotvals, il6_plotvals, age_plotvals, il6_plotvals)
  intVar_vals <- rep(c(rep("age.div10", length(age_plotvals)),
                       rep("il6.log10", length(il6_plotvals))),
                     2)

  ## Things that vary based on biomarker:
  rmsObjName_vals <- c(rep(sprintf("%s.%s.3.mod", endoname, testname),
                           length(age_plotvals) + length(il6_plotvals)),
                       rep(sprintf("%s.%s.12.mod", endoname, testname),
                           length(age_plotvals) + length(il6_plotvals)))
  estVals_val <- rep(list(dd$limits[c(1, 3), paste0(endoname, ".log10")]),
                     (length(age_plotvals) + length(il6_plotvals)) * 2)

  marker_args <- list(intAdjust = intAdjust_vals,
                      rmsObjName = rmsObjName_vals,
                      estVar = paste0(endoname, ".log10"),
                      estVals = estVals_val,
                      intVar = intVar_vals)

  ## Create data frame of all estimates/CIs, add time point
  diffscores <- pmap_df(.l = marker_args, .f = rms_estimate_with_int) %>%
    mutate(timept = c(rep(3, length(age_plotvals) + length(il6_plotvals)),
                      rep(12, length(age_plotvals) + length(il6_plotvals))))

  return(diffscores)
}

## Function to take results of endo_diffs() and get them into plotting shape
endo_plotdata <- function(df){
  ## What order do we want the biomarkers plotted in?
  endo_plotorder <-
    c("bdnf" = 3, "esel" = 4, "pai1" = 5, "s100b" = 1, "uchl" = 2)

  df <- df %>%
    mutate(
      ## Assign Y value to each biomarker
      yval = endo_plotorder[match(gsub("\\.log10$", "", var.main),
                                  names(endo_plotorder))],
      yval_f = factor(sprintf(
        "%s*\n%s vs %s",
        endo.labels[match(gsub("\\.log10", "", var.main), names(endo.labels))],
        rndformat(comp.level),
        rndformat(ref.level)
      )),
      # ## PI-specified order
      # yval_f = forcats::fct_reorder(yval_f, yval, .desc = TRUE),
      ## PI-specified order
      yval_f = forcats::fct_reorder(yval_f, yval, .desc = TRUE),
      ## Factor version of facet titles
      ## Multiply age * 10
      adjust.int.rnd = ifelse(var.int == "age.div10",
                              rndformat(adjust.int * 10, 0),
                              rndformat(adjust.int)),
      ## Get %ile for each interaction value
      adjust.var = ifelse(var.int == "age.div10", "Age", "IL-6*"),
      adjust.pctile =
        ifelse(var.int == "age.div10",
               names(age_plotvals)[match(adjust.int, age_plotvals)],
               names(il6_plotvals)[match(adjust.int, il6_plotvals)]),
      adjust.pctile = gsub("%", "", adjust.pctile, fixed = TRUE),
      int_facet = sprintf("%s = %s\n(%sth %%ile)",
                          adjust.var, adjust.int.rnd, adjust.pctile),

      time_label = factor(ifelse(timept == 3, 1, 2),
                          levels = 1:2,
                          labels = c("3 Months", "12 Months"))
    ) %>%
    dplyr::select(-adjust.int.rnd, -adjust.pctile)
  return(df)
}

rbans_diffs <- map_df(endo.names, endo_diffs, testname = "rbans") %>%
  endo_plotdata()
trailb_diffs <- map_df(endo.names, endo_diffs, testname = "trailb") %>%
  endo_plotdata()

## -- Function to *plot* long-term outcome differences or IRRs, given df -------
## -- from endo_plotdata() -----------------------------------------------------
plot_ltci_forest <- function(forestdf,
                             testname,
                             forestlims,
                             forestbreaks,
                             foresttype = c("cog", "func")){
  foresttype <- match.arg(foresttype)

  if(foresttype == "cog"){
    ytitle <- paste("Adjusted Difference in", testname, "Scores")
    refline <- 0
  } else{
    ytitle <- paste("Adjusted Incidence Rate Ratio for", testname, "Scores")
    refline <- 1
  }

  ggplot(data = forestdf, aes(x = yval_f, y = effect)) +
  geom_hline(yintercept = refline, linetype = "solid", colour = "grey75",
             size = 1, alpha = 0.6) +
  facet_grid(time_label ~ int_facet) +
  geom_pointrange(aes(ymin = lowercl, ymax = uppercl), colour = "#003D79", size = 0.5) +
  # scale_x_discrete(name = "") +
  scale_y_continuous(name = ytitle, limits = forestlims, breaks = forestbreaks) +
  coord_flip() +
  labs(caption = "\n* All biomarkers are transformed using a logarithm, base 10.") +
  theme_minimal() +
...
rbans_diffplot_il6 <- plot_ltci_forest(
  rbans_diffs %>% filter(var.int == "il6.log10"),
  testname = "RBANS Global",
  forestlims = c(-10, 4),
  forestbreaks = seq(-9, 3, 3)
)

trailb_diffplot_il6 <- plot_ltci_forest(
  trailb_diffs %>% filter(var.int == "il6.log10"),
  testname = "Trails B",
  forestlims = c(-7, 4),
  forestbreaks = seq(-6, 4, 2)
)

rbans_diffplot_age <- plot_ltci_forest(
  rbans_diffs %>% filter(var.int == "age.div10"),
  testname = "RBANS Global",
  forestlims = c(-6, 4),
  forestbreaks = seq(-6, 3, 3)
)

trailb_diffplot_age <- plot_ltci_forest(
  trailb_diffs %>% filter(var.int == "age.div10"),
  testname = "Trails B",
  forestlims = c(-6, 8),
  forestbreaks = seq(-6, 8, 2)
)

pdf(file = "brain_endobbb_rbansdiffs_il6.pdf", width = 10, height = 7)
rbans_diffplot_il6
dev.off()

pdf(file = "brain_endobbb_trailbdiffs_il6.pdf", width = 10, height = 7)
trailb_diffplot_il6
dev.off()

pdf(file = "brain_endobbb_rbansdiffs_age.pdf", width = 10, height = 7)
rbans_diffplot_age
dev.off()

pdf(file = "brain_endobbb_trailbdiffs_age.pdf", width = 10, height = 7)
trailb_diffplot_age
dev.off()

## -- Functional outcomes ------------------------------------------------------
## Wrapper for calc.ratioci.endobbb() that creates a data.frame with IRR point
## estimate, CI, and all related values
create_ratioci_plotdata <- function(marker,
                                    test,
                                    timept,
                                    intvar,
                                    ageadj,
  ...
  ## Get model object to use; extract coefficients, vcov matrix
  modobj <- get(paste(marker, test, "il6", timept, sep = "."))$final.mod
  usecoefs <- pool(modobj)$qbar
  usevcov <- pool(modobj)$t

  ## Set marker adjustment values
  markeradjvals <- as.numeric(bio.ratiovals[[paste0(marker, ".log10")]])
  markerpadjvals <- as.numeric(biop.ratiovals[[paste0(marker, ".log10")]])

  tmp <- calc.ratioci.endobbb(
    marker = marker,
    marker.adjust = markeradjvals,
    markerp.adjust = markerpadjvals,
    age.adjust = ageadj,
    il6.adjust = il6adj,
    il6p.adjust = il6padj,
    modcoefs = usecoefs,
    ...
    mutate(
      marker = marker,
      var.main = paste0(marker, ".log10"),
        ## this seems unnecessary but makes it work with endo_plotdata(), which
        ## is built to work w/ rms_estimate_with_int(). Path of least resistance
      test = test,
      timept = timept,
      ref.level = markeradjvals[1],
      comp.level = markeradjvals[2],
      var.int = intvar,
      ageval = ageadj,
      il6val = il6adj,
      adjust.int = ifelse(intvar == "age.div10", ageadj, il6adj)
    ) %>%
    rename(effect = pointest,
           lowercl = lcl,
           uppercl = ucl)
...
##  every test -> (x2)
##  each time point -> (x2)
##  each interacting variable -> (x2)
##  each interacting value (x4)
ntests <- 2
ntimes <- 2
nmarkers <- length(endo.names)
nil6vals <- length(il6_plotvals)
nagevals <- length(age_plotvals)
nintvals <- nil6vals + nagevals

dis_forest_args <- list(
  "marker" = rep(endo.names, each = ntests * ntimes * nintvals),
  "test" = rep(c(rep("adl", ntimes * nintvals), rep("faq", ntimes * nintvals)),
               nmarkers),
  "timept" = rep(c(rep(3, nintvals), rep(12, nintvals)), ntests * nmarkers),
  "intvar" = rep(c(rep("il6.log10", nil6vals),
                   rep("age.div10", nagevals)),
                 ntimes * ntests * nmarkers),
  ## Age: Use median for calculations where IL-6 is varying
  "ageadj" = rep(c(rep(default.adj.vals$age.div10, nil6vals), age_plotvals),
                 ntimes * ntests * nmarkers),
  ## IL-6: Use median for calculations where age is varying
  "il6adj" = rep(c(il6_plotvals, rep(default.adj.vals$il6.log10, nagevals)),
                 ntimes * ntests * nmarkers),
  "il6padj" = rep(c(il6p_plotvals, rep(default.adj.vals$il6.prime, nagevals)),
                  ntimes * ntests * nmarkers)
)

## Make sure everything is as I desire
# dis_forest_args %>% bind_cols() %>% print(n = nrow(.))

dis_forest_data <- pmap_df(dis_forest_args, create_ratioci_plotdata) %>%
  endo_plotdata()

adl_irrplot_il6 <- plot_ltci_forest(
  dis_forest_data %>% filter(var.int == "il6.log10" & test == "adl"),
  testname = "Katz ADL",
  forestlims = c(0, 3.5),
  forestbreaks = seq(0, 3, 1),
  foresttype = "func"
)

faq_irrplot_il6 <- plot_ltci_forest(
  dis_forest_data %>% filter(var.int == "il6.log10" & test == "faq"),
  testname = "FAQ",
  forestlims = c(0.5, 2),
  forestbreaks = seq(0.5, 2, 0.5),
  foresttype = "func"
)

adl_irrplot_age <- plot_ltci_forest(
  dis_forest_data %>% filter(var.int == "age.div10" & test == "adl"),
  testname = "Katz ADL",
  forestlims = c(0, 3),
  forestbreaks = seq(0, 3, 1),
  foresttype = "func"
...
faq_irrplot_age <- plot_ltci_forest(
  dis_forest_data %>% filter(var.int == "age.div10" & test == "faq"),
  testname = "FAQ",
  forestlims = c(0.5, 2),
  forestbreaks = seq(0.5, 2, 0.5),
  foresttype = "func"
...
pdf(file = "brain_endobbb_adlirrs_il6.pdf", width = 10, height = 7)
adl_irrplot_il6
dev.off()

pdf(file = "brain_endobbb_faqirrs_il6.pdf", width = 10, height = 7)
faq_irrplot_il6
dev.off()

pdf(file = "brain_endobbb_adlirrs_age.pdf", width = 10, height = 7)
adl_irrplot_age
dev.off()

pdf(file = "brain_endobbb_faqirrs_age.pdf", width = 10, height = 7)
faq_irrplot_age
dev.off()

@

<<publication_predprobs>>=
## -- Function to get predicted LTCI scores for a given combo of predictors ----
## Arguments: biomarker; test; time point; interacting var; interacting value
pred_bio_ltci <- function(marker = endo.names,
                          test = c("rbans", "trailb"),
                          timept = c(3, 12),
                          intvar = c("il6.log10", "age.div10"),
                          intval,
                          intpct){

  modname <- paste(c(marker, test, timept, "mod"), collapse = ".")

  tmp <- eval(
    parse(
      text = sprintf("as.data.frame(Predict(%s, %s.log10 = NA, %s = %s))",
                     modname,
                     marker,
                     intvar,
                     intval)
  ...
  tmp <- tmp %>%
    mutate(
      ...
      intvar = intvar,
      intpct = intpct,
      ## Factor versions for faceting, titles
      timept_f = factor(timept,
                        levels = c(3, 12),
                        labels = paste(c(3, 12), "Months")),
      intvar_f = ifelse(intvar == "il6.log10", "IL-6*", "Age"),
      marker_f = endo.labels[match(marker, names(endo.labels))]
    )

  ## Change names so that things can be easily put together
  names(tmp) <- gsub(paste0(marker, ".log10"), "markerval", names(tmp))
  names(tmp) <- gsub(intvar, "intvalue", names(tmp))

  tmp %>%
    dplyr::select(test, timept, timept_f, marker, marker_f, markerval,
                  intvar, intvar_f, intvalue, intpct, #intpct_f,
                  yhat, lower, upper)
}
...
pp_args <- list(
  "marker" = rep(endo.names, each = ntests * ntimes * nintvals),
  "test" = rep(c(rep("rbans", ntimes * nintvals),
                 rep("trailb", ntimes * nintvals)),
               nmarkers),
  "timept" = rep(c(rep(3, nintvals), rep(12, nintvals)), ntests * nmarkers),
  "intvar" = rep(c(rep("il6.log10", nil6vals),
                   rep("age.div10", nagevals)),
                 ntimes * ntests * nmarkers),
  "intval" = rep(c(il6_plotvals, age_plotvals), ntimes * ntests * nmarkers),
  "intpct" = gsub("%", "",
                  rep(c(names(il6_plotvals), names(age_plotvals)),
                      ntimes * ntests * nmarkers),
                  fixed = TRUE)
)

# ## Make sure everything is as I want it
# pp_args %>% bind_cols() %>% print(n = nrow(.))

pp_data <- pmap_df(.l = pp_args, .f = pred_bio_ltci)

## -- Function to *plot* RBANS, Trails B data given df from endo_plotdata() ----
plot_ltci_pp <- function(ppdata){
  ppdata <- ppdata %>%
    mutate(
      intvar_val =
        ifelse(intvar_f == "IL-6*",
               paste(intvar_f, round(intvalue, 2), sep = " = "),
               paste(intvar_f, round(intvalue * 10, 0), sep = " = ")),
      intvar_val = paste0(intvar_val, "\n(", intpct, "th %ile)")
    )

  if(unique(ppdata$test) == "rbans"){
    test_title <- "RBANS Global Score"
  } else{
    test_title <- "Trails B T-Score"
  }

  marker <- unique(ppdata$marker)
  marker_label <- endo.labels[match(marker, names(endo.labels))]

  ggplot(data = ppdata, aes(x = markerval, y = yhat)) +
    facet_grid(timept_f ~ intvar_val) +
    geom_ribbon(aes(ymin = lower, ymax = upper), fill = "#003D79", alpha = 0.2) +
    geom_line(color = "#003D79") +
    scale_x_continuous(name = paste0("\n", marker_label, " Plasma Concentration*")) +
    scale_y_continuous(name = paste("\nAdjusted", test_title)) +
    labs(caption = "\n* All biomarkers are transformed using a logarithm, base 10.") +
    theme_minimal() +
...
## Plots for biomarkers with interactions:
## - S100B, RBANS
## - E-selectin, RBANS
## - S100B, Trails B

s100b_rbans_il6_ppplot <- plot_ltci_pp(
  pp_data %>% filter(marker == "s100b" & test == "rbans" & intvar == "il6.log10")
)
pdf(file = "brain_endobbb_s100brbans_il6.pdf", width = 10, height = 7)
s100b_rbans_il6_ppplot
dev.off()

s100b_rbans_age_ppplot <- plot_ltci_pp(
  pp_data %>% filter(marker == "s100b" & test == "rbans" & intvar == "age.div10")
)
pdf(file = "brain_endobbb_s100brbans_age.pdf", width = 10, height = 7)
s100b_rbans_age_ppplot
dev.off()

s100b_trailb_il6_ppplot <- plot_ltci_pp(
  pp_data %>% filter(marker == "s100b" & test == "trailb" & intvar == "il6.log10")
)
pdf(file = "brain_endobbb_s100btrailb_il6.pdf", width = 10, height = 7)
s100b_trailb_il6_ppplot
dev.off()

s100b_trailb_age_ppplot <- plot_ltci_pp(
  pp_data %>% filter(marker == "s100b" & test == "trailb" & intvar == "age.div10")
)
pdf(file = "brain_endobbb_s100btrailb_age.pdf", width = 10, height = 7)
s100b_trailb_age_ppplot
dev.off()

esel_rbans_il6_ppplot <- plot_ltci_pp(
  pp_data %>% filter(marker == "esel" & test == "rbans" & intvar == "il6.log10")
)
pdf(file = "brain_endobbb_eselbrbans_il6.pdf", width = 10, height = 7)
esel_rbans_il6_ppplot
dev.off()

esel_rbans_age_ppplot <- plot_ltci_pp(
  pp_data %>% filter(marker == "esel" & test == "rbans" & intvar == "age.div10")
)
pdf(file = "brain_endobbb_eselbrbans_age.pdf", width = 10, height = 7)
esel_rbans_age_ppplot
dev.off()
...
\clearpage
\section{Sensitivity Analyses: Including Delirium as a Covariate (LTCI Only)}
\subsection{Endothelial/BBB Injury Markers vs. RBANS Global Score}
...
\item Mean daily modified SOFA (no GCS) during ICU stay
\item Duration of severe sepsis
\item \textbf{Duration of delirium}
\item Duration of coma
\end{itemize}
...
<<rbansmodels_sens>>=
## Formula for full delirium model, subbing "marker" in place of actual biomarker prefix
cog.righthand.sens <- "rcs(charlson.score, 3) + rcs(edu.div10, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3)*rcs(marker.log10, 3) + rcs(il6.log10, 3)*rcs(marker.log10, 3)"

## Set datadist for ols() to work
dd <- datadist(endo.data); options(datadist = 'dd')

## 3-month models
fit.rbans.sens.models.3 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.sens,
                 outcome = 'rbans.global.score.3',
                 impobj = areg.3,
                 dataset = endo.data,
                 cohort.var = 'fu.cohort.3',
...
bdnf.rbans.sens.3 <- fit.rbans.sens.models.3('bdnf')
esel.rbans.sens.3 <- fit.rbans.sens.models.3('esel')
pai1.rbans.sens.3 <- fit.rbans.sens.models.3('pai1')
s100b.rbans.sens.3 <- fit.rbans.sens.models.3('s100b')
uchl.rbans.sens.3 <- fit.rbans.sens.models.3('uchl')

# ## Check assumptions of final models
# ols.diagnostics(model.obj.ma = bdnf.rbans.sens.3$final.mod, labeltext = 'BDNF vs RBANS, 3m')
# ols.diagnostics(model.obj.ma = esel.rbans.sens.3$final.mod, labeltext = 'E-selectin vs RBANS, 3m')
# ols.diagnostics(model.obj.ma = pai1.rbans.sens.3$final.mod, labeltext = 'PAI-1 vs RBANS, 3m')
# ols.diagnostics(model.obj.ma = s100b.rbans.sens.3$final.mod, labeltext = 'S100B vs RBANS, 3m')
# ols.diagnostics(model.obj.ma = uchl.rbans.sens.3$final.mod, labeltext = 'UCHL vs RBANS, 3m')

bdnf.rbans.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'bdnf.rbans.sens.3', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.rbans.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'esel.rbans.sens.3', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.rbans.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'pai1.rbans.sens.3', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.rbans.sens.3.fig <-
  ols.figures.rbans.3(olslist = 's100b.rbans.sens.3', xvar = 's100b.log10', xstring = 'S100B')
uchl.rbans.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'uchl.rbans.sens.3', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_rbans3_sens_results.pdf', width = 8, height = bdnf.rbans.sens.3.fig$height)
multiplot(plotlist = bdnf.rbans.sens.3.fig$fig, layout = bdnf.rbans.sens.3.fig$layoutmat)
dev.off()

pdf(file = 'esel_rbans3_sens_results.pdf', width = 8, height = esel.rbans.sens.3.fig$height)
multiplot(plotlist = esel.rbans.sens.3.fig$fig, layout = esel.rbans.sens.3.fig$layoutmat)
dev.off()

pdf(file = 'pai1_rbans3_sens_results.pdf', width = 8, height = pai1.rbans.sens.3.fig$height)
multiplot(plotlist = pai1.rbans.sens.3.fig$fig, layout = pai1.rbans.sens.3.fig$layoutmat)
dev.off()

pdf(file = 's100b_rbans3_sens_results.pdf', width = 8, height = s100b.rbans.sens.3.fig$height)
multiplot(plotlist = s100b.rbans.sens.3.fig$fig, layout = s100b.rbans.sens.3.fig$layoutmat)
dev.off()

pdf(file = 'uchl_rbans3_sens_results.pdf', width = 8, height = uchl.rbans.sens.3.fig$height)
multiplot(plotlist = uchl.rbans.sens.3.fig$fig, layout = uchl.rbans.sens.3.fig$layoutmat)
dev.off()

## 12-month models
fit.rbans.sens.models.12 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.sens,
                 outcome = 'rbans.global.score.12',
                 impobj = areg.12,
                 dataset = endo.data,
...
bdnf.rbans.sens.12 <- fit.rbans.sens.models.12('bdnf')
esel.rbans.sens.12 <- fit.rbans.sens.models.12('esel')
pai1.rbans.sens.12 <- fit.rbans.sens.models.12('pai1')
s100b.rbans.sens.12 <- fit.rbans.sens.models.12('s100b')
uchl.rbans.sens.12 <- fit.rbans.sens.models.12('uchl')

# ## Check assumptions of final models
# ols.diagnostics(model.obj.ma = bdnf.rbans.sens.12$final.mod, labeltext = 'BDNF vs RBANS, 12m')
# ols.diagnostics(model.obj.ma = esel.rbans.sens.12$final.mod, labeltext = 'E-selectin vs RBANS, 12m')
# ols.diagnostics(model.obj.ma = pai1.rbans.sens.12$final.mod, labeltext = 'PAI-1 vs RBANS, 12m')
# ols.diagnostics(model.obj.ma = s100b.rbans.sens.12$final.mod, labeltext = 'S100B vs RBANS, 12m')
# ols.diagnostics(model.obj.ma = uchl.rbans.sens.12$final.mod, labeltext = 'UCHL vs RBANS, 12m')
...
#
# ## Nonlinear terms needed; test interactions
# s100b.rbans.sens.3$p.int
#
# ## Interactions needed; test marker, IL-6
# s100b.rbans.sens.3$p.marker
# s100b.rbans.sens.3$p.il6

bdnf.rbans.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'bdnf.rbans.sens.12', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.rbans.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'esel.rbans.sens.12', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.rbans.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'pai1.rbans.sens.12', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.rbans.sens.12.fig <-
  ols.figures.rbans.12(olslist = 's100b.rbans.sens.12', xvar = 's100b.log10', xstring = 'S100B')
uchl.rbans.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'uchl.rbans.sens.12', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_rbans12_sens_results.pdf', width = 8, height = bdnf.rbans.sens.12.fig$height)
multiplot(plotlist = bdnf.rbans.sens.12.fig$fig, layout = bdnf.rbans.sens.12.fig$layoutmat)
dev.off()

pdf(file = 'esel_rbans12_sens_results.pdf', width = 8, height = esel.rbans.sens.12.fig$height)
multiplot(plotlist = esel.rbans.sens.12.fig$fig, layout = esel.rbans.sens.12.fig$layoutmat)
dev.off()

pdf(file = 'pai1_rbans12_sens_results.pdf', width = 8, height = pai1.rbans.sens.12.fig$height)
multiplot(plotlist = pai1.rbans.sens.12.fig$fig, layout = pai1.rbans.sens.12.fig$layoutmat)
dev.off()

pdf(file = 's100b_rbans12_sens_results.pdf', width = 8, height = s100b.rbans.sens.12.fig$height)
multiplot(plotlist = s100b.rbans.sens.12.fig$fig, layout = s100b.rbans.sens.12.fig$layoutmat)
dev.off()

pdf(file = 'uchl_rbans12_sens_results.pdf', width = 8, height = uchl.rbans.sens.12.fig$height)
multiplot(plotlist = uchl.rbans.sens.12.fig$fig, layout = uchl.rbans.sens.12.fig$layoutmat)
dev.off()


## -- Models without IL-6 --------------------------------------------------------------------------
## Formula for full delirium model, subbing "markervar" in place of actual biomarker variable
cog.righthand.noil6.sens <- "rcs(charlson.score, 3) + rcs(edu.div10, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3)*rcs(marker.log10, 3)"

## 3-month models
fit.rbans.noil6.sens.models.3 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.noil6.sens,
                 outcome = 'rbans.global.score.3',
                 impobj = areg.3,
                 dataset = endo.data,
                 cohort.var = 'fu.cohort.3',
...
bdnf.rbans.noil6.sens.3 <- fit.rbans.noil6.sens.models.3('bdnf')
esel.rbans.noil6.sens.3 <- fit.rbans.noil6.sens.models.3('esel')
pai1.rbans.noil6.sens.3 <- fit.rbans.noil6.sens.models.3('pai1')
s100b.rbans.noil6.sens.3 <- fit.rbans.noil6.sens.models.3('s100b')
uchl.rbans.noil6.sens.3 <- fit.rbans.noil6.sens.models.3('uchl')

bdnf.rbans.noil6.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'bdnf.rbans.noil6.sens.3', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.rbans.noil6.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'esel.rbans.noil6.sens.3', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.rbans.noil6.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'pai1.rbans.noil6.sens.3', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.rbans.noil6.sens.3.fig <-
  ols.figures.rbans.3(olslist = 's100b.rbans.noil6.sens.3', xvar = 's100b.log10', xstring = 'S100B')
uchl.rbans.noil6.sens.3.fig <-
  ols.figures.rbans.3(olslist = 'uchl.rbans.noil6.sens.3', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_rbansnoil63_sens_results.pdf', width = 8, height = bdnf.rbans.noil6.sens.3.fig$height)
multiplot(plotlist = bdnf.rbans.noil6.sens.3.fig$fig, layout = bdnf.rbans.noil6.sens.3.fig$layoutmat)
dev.off()

pdf(file = 'esel_rbansnoil63_sens_results.pdf', width = 8, height = esel.rbans.noil6.sens.3.fig$height)
multiplot(plotlist = esel.rbans.noil6.sens.3.fig$fig, layout = esel.rbans.noil6.sens.3.fig$layoutmat)
dev.off()

pdf(file = 'pai1_rbansnoil63_sens_results.pdf', width = 8, height = pai1.rbans.noil6.sens.3.fig$height)
multiplot(plotlist = pai1.rbans.noil6.sens.3.fig$fig, layout = pai1.rbans.noil6.sens.3.fig$layoutmat)
dev.off()

pdf(file = 's100b_rbansnoil63_sens_results.pdf', width = 8, height = s100b.rbans.noil6.sens.3.fig$height)
multiplot(plotlist = s100b.rbans.noil6.sens.3.fig$fig, layout = s100b.rbans.noil6.sens.3.fig$layoutmat)
dev.off()

pdf(file = 'uchl_rbansnoil63_sens_results.pdf', width = 8, height = uchl.rbans.noil6.sens.3.fig$height)
multiplot(plotlist = uchl.rbans.noil6.sens.3.fig$fig, layout = uchl.rbans.noil6.sens.3.fig$layoutmat)
dev.off()

## 12-month models
fit.rbans.noil6.sens.models.12 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.noil6.sens,
                 outcome = 'rbans.global.score.12',
                 impobj = areg.12,
                 dataset = endo.data,
...
bdnf.rbans.noil6.sens.12 <- fit.rbans.noil6.sens.models.12('bdnf')
esel.rbans.noil6.sens.12 <- fit.rbans.noil6.sens.models.12('esel')
pai1.rbans.noil6.sens.12 <- fit.rbans.noil6.sens.models.12('pai1')
s100b.rbans.noil6.sens.12 <- fit.rbans.noil6.sens.models.12('s100b')
uchl.rbans.noil6.sens.12 <- fit.rbans.noil6.sens.models.12('uchl')

bdnf.rbans.noil6.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'bdnf.rbans.noil6.sens.12', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.rbans.noil6.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'esel.rbans.noil6.sens.12', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.rbans.noil6.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'pai1.rbans.noil6.sens.12', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.rbans.noil6.sens.12.fig <-
  ols.figures.rbans.12(olslist = 's100b.rbans.noil6.sens.12', xvar = 's100b.log10', xstring = 'S100B')
uchl.rbans.noil6.sens.12.fig <-
  ols.figures.rbans.12(olslist = 'uchl.rbans.noil6.sens.12', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_rbansnoil612_sens_results.pdf', width = 8, height = bdnf.rbans.noil6.sens.12.fig$height)
multiplot(plotlist = bdnf.rbans.noil6.sens.12.fig$fig, layout = bdnf.rbans.noil6.sens.12.fig$layoutmat)
dev.off()

pdf(file = 'esel_rbansnoil612_sens_results.pdf', width = 8, height = esel.rbans.noil6.sens.12.fig$height)
multiplot(plotlist = esel.rbans.noil6.sens.12.fig$fig, layout = esel.rbans.noil6.sens.12.fig$layoutmat)
dev.off()

pdf(file = 'pai1_rbansnoil612_sens_results.pdf', width = 8, height = pai1.rbans.noil6.sens.12.fig$height)
multiplot(plotlist = pai1.rbans.noil6.sens.12.fig$fig, layout = pai1.rbans.noil6.sens.12.fig$layoutmat)
dev.off()

pdf(file = 's100b_rbansnoil612_sens_results.pdf', width = 8, height = s100b.rbans.noil6.sens.12.fig$height)
multiplot(plotlist = s100b.rbans.noil6.sens.12.fig$fig, layout = s100b.rbans.noil6.sens.12.fig$layoutmat)
dev.off()

pdf(file = 'uchl_rbansnoil612_sens_results.pdf', width = 8, height = uchl.rbans.noil6.sens.12.fig$height)
multiplot(plotlist = uchl.rbans.noil6.sens.12.fig$fig, layout = uchl.rbans.noil6.sens.12.fig$layoutmat)
dev.off()
...
<<printbdnfrbans3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(bdnf.rbans.sens.3, 'BDNF'),
                       create.pval.table(bdnf.rbans.noil6.sens.3, 'BDNF')),
                 caption = 'BDNF vs 3-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printbdnfrbans12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(bdnf.rbans.sens.12, 'BDNF'),
                       create.pval.table(bdnf.rbans.noil6.sens.12, 'BDNF')),
                 caption = 'BDNF vs 12-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\clearpage
<<printbdnfmodresultssens, results = "asis">>=
latex_combined_results(
  bdnf.rbans.sens.3, bdnf.rbans.noil6.sens.3,
  "BDNF vs RBANS (Sensitivity Analyses), 3 Months"
)

latex_combined_results(
  bdnf.rbans.sens.12, bdnf.rbans.noil6.sens.12,
  "BDNF vs RBANS (Sensitivity Analyses), 12 Months"
)

@

\begin{figure}[!h]
{\centering\includegraphics{bdnf_rbans3_sens_results.pdf}}
\caption{Log10(BDNF) vs. RBANS at 3 Months, Adjusting for Log10(IL-6)}\label{fig:bdnfrbans3sens}
{\centering\includegraphics{bdnf_rbansnoil63_sens_results.pdf}}
\caption{Log10(BDNF) vs. RBANS at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:bdnfrbansnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{bdnf_rbans12_sens_results.pdf}}
\caption{Log10(BDNF) vs. RBANS at 12 Months, Adjusting for Log10(IL-6)}\label{fig:bdnfrbans12sens}
{\centering\includegraphics{bdnf_rbansnoil612_sens_results.pdf}}
\caption{Log10(BDNF) vs. RBANS at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:bdnfrbansnoil612sens}\end{figure}
...
<<printeselrbans3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(esel.rbans.sens.3, 'E-selectin'),
                       create.pval.table(esel.rbans.noil6.sens.3, 'E-selectin')),
                 caption = 'E-selectin vs 3-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printeselrbans12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(esel.rbans.sens.12, 'E-selectin'),
                       create.pval.table(esel.rbans.noil6.sens.12, 'E-selectin')),
                 caption = 'E-selectin vs 12-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\clearpage
<<printeselmodresultssens, results = "asis">>=
latex_combined_results(
  esel.rbans.sens.3, esel.rbans.noil6.sens.3,
  "E-selectin vs RBANS (Sensitivity Analyses), 3 Months"
)

latex_combined_results(
  esel.rbans.sens.12, esel.rbans.noil6.sens.12,
  "E-selectin vs RBANS (Sensitivity Analyses), 12 Months"
)

@

\begin{figure}[!h]
{\centering\includegraphics{esel_rbans3_sens_results.pdf}}
\caption{Log10(E-selectin) vs. RBANS at 3 Months, Adjusting for Log10(IL-6)}\label{fig:eselrbans3sens}
{\centering\includegraphics{esel_rbansnoil63_sens_results.pdf}}
\caption{Log10(E-selectin) vs. RBANS at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:eselrbansnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{esel_rbans12_sens_results.pdf}}
\caption{Log10(E-selectin) vs. RBANS at 12 Months, Adjusting for Log10(IL-6)}\label{fig:eselrbans12sens}
{\centering\includegraphics{esel_rbansnoil612_sens_results.pdf}}
\caption{Log10(E-selectin) vs. RBANS at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:eselrbansnoil612sens}\end{figure}

\clearpage
\subsection{PAI-1 vs. RBANS}
We see no meaningful association between log10(PAI-1) and RBANS scores at 3 or 12 months.

<<printpai1rbans3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(pai1.rbans.sens.3, 'PAI-1'),
                       create.pval.table(pai1.rbans.noil6.sens.3, 'PAI-1')),
                 caption = 'PAI-1 vs 3-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printpai1rbans12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(pai1.rbans.sens.12, 'PAI-1'),
                       create.pval.table(pai1.rbans.noil6.sens.12, 'PAI-1')),
                 caption = 'PAI-1 vs 12-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\clearpage
<<printpai1modresultssens, results = "asis">>=
latex_combined_results(
  pai1.rbans.sens.3, pai1.rbans.noil6.sens.3,
  "PAI-1 vs RBANS (Sensitivity Analyses), 3 Months"
)

latex_combined_results(
  pai1.rbans.sens.12, pai1.rbans.noil6.sens.12,
  "PAI-1 vs RBANS (Sensitivity Analyses), 12 Months"
)

@

\begin{figure}[!h]
{\centering\includegraphics{pai1_rbans3_sens_results.pdf}}
\caption{Log10(PAI-1) vs. RBANS at 3 Months, Adjusting for Log10(IL-6)}\label{fig:pai1rbans3sens}
{\centering\includegraphics{pai1_rbansnoil63_sens_results.pdf}}
\caption{Log10(PAI-1) vs. RBANS at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:pai1rbansnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{pai1_rbans12_sens_results.pdf}}
\caption{Log10(PAI-1) vs. RBANS at 12 Months, Adjusting for Log10(IL-6)}\label{fig:pai1rbans12sens}
{\centering\includegraphics{pai1_rbansnoil612_sens_results.pdf}}
\caption{Log10(PAI-1) vs. RBANS at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:pai1rbansnoil612sens}\end{figure}
...
<<prints100brbans3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(s100b.rbans.sens.3, 'S100B'),
                       create.pval.table(s100b.rbans.noil6.sens.3, 'S100B')),
                 caption = 'S100B vs 3-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<prints100brbans12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(s100b.rbans.sens.12, 'S100B'),
                       create.pval.table(s100b.rbans.noil6.sens.12, 'S100B')),
                 caption = 'S100B vs 12-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\clearpage
<<prints100bmodresultssens, results = "asis">>=
latex_combined_results(
  s100b.rbans.sens.3, s100b.rbans.noil6.sens.3,
  "S100B vs RBANS (Sensitivity Analyses), 3 Months"
)

latex_combined_results(
  s100b.rbans.sens.12, s100b.rbans.noil6.sens.12,
  "S100B vs RBANS (Sensitivity Analyses), 12 Months"
)

@

\begin{figure}[!h]
{\centering\includegraphics{s100b_rbans3_sens_results.pdf}}
\caption{Log10(S100B) vs. RBANS at 3 Months, Adjusting for Log10(IL-6)}\label{fig:s100brbans3sens}
{\centering\includegraphics{s100b_rbansnoil63_sens_results.pdf}}
\caption{Log10(S100B) vs. RBANS at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:s100brbansnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{s100b_rbans12_sens_results.pdf}}
\caption{Log10(S100B) vs. RBANS at 12 Months, Adjusting for Log10(IL-6)}\label{fig:s100brbans12sens}
{\centering\includegraphics{s100b_rbansnoil612_sens_results.pdf}}
\caption{Log10(S100B) vs. RBANS at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:s100brbansnoil612sens}\end{figure}

\clearpage
\subsection{UCHL vs. RBANS}
We see a significant association between log10(UCHL) at baseline and RBANS  scores at 3-month followup. By 12-month followup, however, this association is no longer present.

<<printuchlrbans3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(uchl.rbans.sens.3, 'UCHL'),
                       create.pval.table(uchl.rbans.noil6.sens.3, 'UCHL')),
                 caption = 'UCHL vs 3-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printuchlrbans12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(uchl.rbans.sens.12, 'UCHL'),
                       create.pval.table(uchl.rbans.noil6.sens.12, 'UCHL')),
                 caption = 'UCHl vs 12-Month RBANS',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\clearpage
<<printuchlmodresultssens, results = "asis">>=
latex_combined_results(
  uchl.rbans.sens.3, uchl.rbans.noil6.sens.3,
  "UCHL vs RBANS (Sensitivity Analyses), 3 Months"
)

latex_combined_results(
  uchl.rbans.sens.12, uchl.rbans.noil6.sens.12,
  "UCHL vs RBANS (Sensitivity Analyses), 12 Months"
)

@

\begin{figure}[!h]
{\centering\includegraphics{uchl_rbans3_sens_results.pdf}}
\caption{Log10(UCHL) vs. RBANS at 3 Months, Adjusting for Log10(IL-6)}\label{fig:uchlrbans3sens}
{\centering\includegraphics{uchl_rbansnoil63_sens_results.pdf}}
\caption{Log10(UCHL) vs. RBANS at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:uchlrbansnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{uchl_rbans12_sens_results.pdf}}
\caption{Log10(UCHL) vs. RBANS at 12 Months, Adjusting for Log10(IL-6)}\label{fig:uchlrbans12sens}
{\centering\includegraphics{uchl_rbansnoil612_sens_results.pdf}}
\caption{Log10(UCHL) vs. RBANS at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:uchlrbansnoil612sens}\end{figure}
...
<<trailbmodelssens>>=
## 3-month models
fit.trailb.models.sens.3 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.sens,
                 outcome = 'trail.b.tscore.3',
                 impobj = areg.3,
                 dataset = endo.data,
...
bdnf.trailb.sens.3 <- fit.trailb.models.sens.3('bdnf')
esel.trailb.sens.3 <- fit.trailb.models.sens.3('esel')
pai1.trailb.sens.3 <- fit.trailb.models.sens.3('pai1')
s100b.trailb.sens.3 <- fit.trailb.models.sens.3('s100b')
uchl.trailb.sens.3 <- fit.trailb.models.sens.3('uchl')

# ## Check assumptions of final models
# ols.diagnostics(model.obj.ma = bdnf.trailb.sens.3$final.mod, labeltext = 'BDNF vs Trails B, 3m')
# ols.diagnostics(model.obj.ma = esel.trailb.sens.3$final.mod, labeltext = 'E-selectin vs Trails B, 3m')
# ols.diagnostics(model.obj.ma = pai1.trailb.sens.3$final.mod, labeltext = 'PAI-1 vs Trails B, 3m')
# ols.diagnostics(model.obj.ma = s100b.trailb.sens.3$final.mod, labeltext = 'S100B vs Trails B, 3m')
# ols.diagnostics(model.obj.ma = uchl.trailb.sens.3$final.mod, labeltext = 'UCHL vs Trails B, 3m')

bdnf.trailb.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'bdnf.trailb.sens.3', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.trailb.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'esel.trailb.sens.3', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.trailb.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'pai1.trailb.sens.3', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.trailb.3.sens.fig <-
  ols.figures.trailb.3(olslist = 's100b.trailb.sens.3', xvar = 's100b.log10', xstring = 'S100B')
uchl.trailb.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'uchl.trailb.sens.3', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_trailb3_sens_results.pdf', width = 8, height = bdnf.trailb.3.sens.fig$height)
multiplot(plotlist = bdnf.trailb.3.sens.fig$fig, layout = bdnf.trailb.3.sens.fig$layoutmat)
dev.off()

pdf(file = 'esel_trailb3_sens_results.pdf', width = 8, height = esel.trailb.3.sens.fig$height)
multiplot(plotlist = esel.trailb.3.sens.fig$fig, layout = esel.trailb.3.sens.fig$layoutmat)
dev.off()

pdf(file = 'pai1_trailb3_sens_results.pdf', width = 8, height = pai1.trailb.3.sens.fig$height)
multiplot(plotlist = pai1.trailb.3.sens.fig$fig, layout = pai1.trailb.3.sens.fig$layoutmat)
dev.off()

pdf(file = 's100b_trailb3_sens_results.pdf', width = 8, height = s100b.trailb.3.sens.fig$height)
multiplot(plotlist = s100b.trailb.3.sens.fig$fig, layout = s100b.trailb.3.sens.fig$layoutmat)
dev.off()

pdf(file = 'uchl_trailb3_sens_results.pdf', width = 8, height = uchl.trailb.3.sens.fig$height)
multiplot(plotlist = uchl.trailb.3.sens.fig$fig, layout = uchl.trailb.3.sens.fig$layoutmat)
dev.off()

## 12-month models
fit.trailb.models.sens.12 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.sens,
                 outcome = 'trail.b.tscore.12',
                 impobj = areg.12,
                 dataset = endo.data,
...
bdnf.trailb.sens.12 <- fit.trailb.models.sens.12('bdnf')
esel.trailb.sens.12 <- fit.trailb.models.sens.12('esel')
pai1.trailb.sens.12 <- fit.trailb.models.sens.12('pai1')
s100b.trailb.sens.12 <- fit.trailb.models.sens.12('s100b')
uchl.trailb.sens.12 <- fit.trailb.models.sens.12('uchl')

# ## Check assumptions of final models
# ols.diagnostics(model.obj.ma = bdnf.trailb.sens.12$final.mod, labeltext = 'BDNF vs Trails B, 12m')
# ols.diagnostics(model.obj.ma = esel.trailb.sens.12$final.mod, labeltext = 'E-selectin vs Trails B, 12m')
# ols.diagnostics(model.obj.ma = pai1.trailb.sens.12$final.mod, labeltext = 'PAI-1 vs Trails B, 12m')
# ols.diagnostics(model.obj.ma = s100b.trailb.sens.12$final.mod, labeltext = 'S100B vs Trails B, 12m')
# ols.diagnostics(model.obj.ma = uchl.trailb.sens.12$final.mod, labeltext = 'UCHL vs Trails B, 12m')

bdnf.trailb.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'bdnf.trailb.sens.12', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.trailb.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'esel.trailb.sens.12', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.trailb.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'pai1.trailb.sens.12', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.trailb.12.sens.fig <-
  ols.figures.trailb.12(olslist = 's100b.trailb.sens.12', xvar = 's100b.log10', xstring = 'S100B')
uchl.trailb.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'uchl.trailb.sens.12', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_trailb12_sens_results.pdf', width = 8, height = bdnf.trailb.12.sens.fig$height)
multiplot(plotlist = bdnf.trailb.12.sens.fig$fig, layout = bdnf.trailb.12.sens.fig$layoutmat)
dev.off()

pdf(file = 'esel_trailb12_sens_results.pdf', width = 8, height = esel.trailb.12.sens.fig$height)
multiplot(plotlist = esel.trailb.12.sens.fig$fig, layout = esel.trailb.12.sens.fig$layoutmat)
dev.off()

pdf(file = 'pai1_trailb12_sens_results.pdf', width = 8, height = pai1.trailb.12.sens.fig$height)
multiplot(plotlist = pai1.trailb.12.sens.fig$fig, layout = pai1.trailb.12.sens.fig$layoutmat)
dev.off()

pdf(file = 's100b_trailb12_sens_results.pdf', width = 8, height = s100b.trailb.12.sens.fig$height)
multiplot(plotlist = s100b.trailb.12.sens.fig$fig, layout = s100b.trailb.12.sens.fig$layoutmat)
dev.off()

pdf(file = 'uchl_trailb12_sens_results.pdf', width = 8, height = uchl.trailb.12.fig$height)
multiplot(plotlist = uchl.trailb.12.sens.fig$fig, layout = uchl.trailb.12.sens.fig$layoutmat)
dev.off()


## -- Models without IL-6 --------------------------------------------------------------------------
## 3-month models
fit.trailb.noil6.models.sens.3 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.noil6.sens,
                 outcome = 'trail.b.tscore.3',
                 impobj = areg.3,
                 dataset = endo.data,
...
bdnf.trailb.noil6.sens.3 <- fit.trailb.noil6.models.sens.3('bdnf')
esel.trailb.noil6.sens.3 <- fit.trailb.noil6.models.sens.3('esel')
pai1.trailb.noil6.sens.3 <- fit.trailb.noil6.models.sens.3('pai1')
s100b.trailb.noil6.sens.3 <- fit.trailb.noil6.models.sens.3('s100b')
uchl.trailb.noil6.sens.3 <- fit.trailb.noil6.models.sens.3('uchl')

bdnf.trailb.noil6.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'bdnf.trailb.noil6.sens.3', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.trailb.noil6.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'esel.trailb.noil6.sens.3', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.trailb.noil6.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'pai1.trailb.noil6.sens.3', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.trailb.noil6.3.sens.fig <-
  ols.figures.trailb.3(olslist = 's100b.trailb.noil6.sens.3', xvar = 's100b.log10', xstring = 'S100B')
uchl.trailb.noil6.3.sens.fig <-
  ols.figures.trailb.3(olslist = 'uchl.trailb.noil6.sens.3', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_trailbnoil63_sens_results.pdf', width = 8, height = bdnf.trailb.noil6.3.sens.fig$height)
multiplot(plotlist = bdnf.trailb.noil6.3.sens.fig$fig, layout = bdnf.trailb.noil6.3.sens.fig$layoutmat)
dev.off()

pdf(file = 'esel_trailbnoil63_sens_results.pdf', width = 8, height = esel.trailb.noil6.3.sens.fig$height)
multiplot(plotlist = esel.trailb.noil6.3.sens.fig$fig, layout = esel.trailb.noil6.3.sens.fig$layoutmat)
dev.off()

pdf(file = 'pai1_trailbnoil63_sens_results.pdf', width = 8, height = pai1.trailb.noil6.3.sens.fig$height)
multiplot(plotlist = pai1.trailb.noil6.3.sens.fig$fig, layout = pai1.trailb.noil6.3.sens.fig$layoutmat)
dev.off()

pdf(file = 's100b_trailbnoil63_sens_results.pdf', width = 8, height = s100b.trailb.noil6.3.sens.fig$height)
multiplot(plotlist = s100b.trailb.noil6.3.sens.fig$fig, layout = s100b.trailb.noil6.3.sens.fig$layoutmat)
dev.off()

pdf(file = 'uchl_trailbnoil63_sens_results.pdf', width = 8, height = uchl.trailb.noil6.3.sens.fig$height)
multiplot(plotlist = uchl.trailb.noil6.3.sens.fig$fig, layout = uchl.trailb.noil6.3.sens.fig$layoutmat)
dev.off()

## 12-month models
fit.trailb.noil6.models.sens.12 <- function(marker){
  fit.ols.models(formula.right = cog.righthand.noil6.sens,
                 outcome = 'trail.b.tscore.12',
                 impobj = areg.12,
                 dataset = endo.data,
...
bdnf.trailb.noil6.sens.12 <- fit.trailb.noil6.models.sens.12('bdnf')
esel.trailb.noil6.sens.12 <- fit.trailb.noil6.models.sens.12('esel')
pai1.trailb.noil6.sens.12 <- fit.trailb.noil6.models.sens.12('pai1')
s100b.trailb.noil6.sens.12 <- fit.trailb.noil6.models.sens.12('s100b')
uchl.trailb.noil6.sens.12 <- fit.trailb.noil6.models.sens.12('uchl')

bdnf.trailb.noil6.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'bdnf.trailb.noil6.sens.12', xvar = 'bdnf.log10', xstring = 'BDNF')
esel.trailb.noil6.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'esel.trailb.noil6.sens.12', xvar = 'esel.log10', xstring = 'E-selectin')
pai1.trailb.noil6.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'pai1.trailb.noil6.sens.12', xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.trailb.noil6.12.sens.fig <-
  ols.figures.trailb.12(olslist = 's100b.trailb.noil6.sens.12', xvar = 's100b.log10', xstring = 'S100B')
uchl.trailb.noil6.12.sens.fig <-
  ols.figures.trailb.12(olslist = 'uchl.trailb.noil6.sens.12', xvar = 'uchl.log10', xstring = 'UCHL')

pdf(file = 'bdnf_trailbnoil612_sens_results.pdf', width = 8, height = bdnf.trailb.noil6.12.sens.fig$height)
multiplot(plotlist = bdnf.trailb.noil6.12.sens.fig$fig, layout = bdnf.trailb.noil6.12.sens.fig$layoutmat)
dev.off()

pdf(file = 'esel_trailbnoil612_sens_results.pdf', width = 8, height = esel.trailb.noil6.12.sens.fig$height)
multiplot(plotlist = esel.trailb.noil6.12.sens.fig$fig, layout = esel.trailb.noil6.12.sens.fig$layoutmat)
dev.off()

pdf(file = 'pai1_trailbnoil612_sens_results.pdf', width = 8, height = pai1.trailb.noil6.12.sens.fig$height)
multiplot(plotlist = pai1.trailb.noil6.12.sens.fig$fig, layout = pai1.trailb.noil6.12.sens.fig$layoutmat)
dev.off()

pdf(file = 's100b_trailbnoil612_sens_results.pdf', width = 8, height = s100b.trailb.noil6.12.sens.fig$height)
multiplot(plotlist = s100b.trailb.noil6.12.sens.fig$fig, layout = s100b.trailb.noil6.12.sens.fig$layoutmat)
dev.off()

pdf(file = 'uchl_trailbnoil612_sens_results.pdf', width = 8, height = uchl.trailb.noil6.12.sens.fig$height)
multiplot(plotlist = uchl.trailb.noil6.12.sens.fig$fig, layout = uchl.trailb.noil6.12.sens.fig$layoutmat)
dev.off()
...
<<printbdnftrailb3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(bdnf.trailb.sens.3, 'BDNF'),
                       create.pval.table(bdnf.trailb.noil6.sens.3, 'BDNF')),
                 caption = 'BDNF vs 3-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printbdnftrailb12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(bdnf.trailb.sens.12, 'BDNF'),
                       create.pval.table(bdnf.trailb.noil6.sens.12, 'BDNF')),
                 caption = 'BDNF vs 12-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\begin{figure}[!h]
{\centering\includegraphics{bdnf_trailb3_sens_results.pdf}}
\caption{Log10(BDNF) vs. Trails B at 3 Months, Adjusting for Log10(IL-6)}\label{fig:bdnftrailb3sens}
{\centering\includegraphics{bdnf_trailbnoil63_sens_results.pdf}}
\caption{Log10(BDNF) vs. Trails B at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:bdnftrailbnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{bdnf_trailb12_sens_results.pdf}}
\caption{Log10(BDNF) vs. Trails B at 12 Months, Adjusting for Log10(IL-6)}\label{fig:bdnftrailb12sens}
{\centering\includegraphics{bdnf_trailbnoil612_sens_results.pdf}}
\caption{Log10(BDNF) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:bdnftrailbnoil612sens}\end{figure}
...
<<printeseltrailb3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(esel.trailb.sens.3, 'E-selectin'),
                       create.pval.table(esel.trailb.noil6.sens.3, 'E-selectin')),
                 caption = 'E-selectin vs 3-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printeseltrailb12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(esel.trailb.sens.12, 'E-selectin'),
                       create.pval.table(esel.trailb.noil6.sens.12, 'E-selectin')),
                 caption = 'E-selectin vs 12-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\begin{figure}[!h]
{\centering\includegraphics{esel_trailb3_sens_results.pdf}}
\caption{Log10(E-selectin) vs. Trails B at 3 Months, Adjusting for Log10(IL-6)}\label{fig:eseltrailb3sens}
{\centering\includegraphics{esel_trailbnoil63_sens_results.pdf}}
\caption{Log10(E-selectin) vs. Trails B at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:eseltrailbnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{esel_trailb12_sens_results.pdf}}
\caption{Log10(E-selectin) vs. Trails B at 12 Months, Adjusting for Log10(IL-6)}\label{fig:eseltrailb12sens}
{\centering\includegraphics{esel_trailbnoil612_sens_results.pdf}}
\caption{Log10(E-selectin) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:eseltrailbnoil612sens}\end{figure}

\clearpage
\subsection{PAI-1 vs. Trails B}
We see no meaningful association between log10(PAI-1) and Trails B t-scores at 3 months; at 12 months, we see a marginally significant association such that higher log10(PAI-1) is marginally associated with higher Trails B scores.

<<printpai1trailb3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(pai1.trailb.sens.3, 'PAI-1'),
                       create.pval.table(pai1.trailb.noil6.sens.3, 'PAI-1')),
                 caption = 'PAI-1 vs 3-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printpai1trailb12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(pai1.trailb.sens.12, 'PAI-1'),
                       create.pval.table(pai1.trailb.noil6.sens.12, 'PAI-1')),
                 caption = 'PAI-1 vs 12-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\begin{figure}[!h]
{\centering\includegraphics{pai1_trailb3_sens_results.pdf}}
\caption{Log10(PAI-1) vs. Trails B at 3 Months, Adjusting for Log10(IL-6)}\label{fig:pai1trailb3sens}
{\centering\includegraphics{pai1_trailbnoil63_sens_results.pdf}}
\caption{Log10(PAI-1) vs. Trails B at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:pai1trailbnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{pai1_trailb12_sens_results.pdf}}
\caption{Log10(PAI-1) vs. Trails B at 12 Months, Adjusting for Log10(IL-6)}\label{fig:pai1trailb12sens}
{\centering\includegraphics{pai1_trailbnoil612_sens_results.pdf}}
\caption{Log10(PAI-1) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:pai1trailbnoil612sens}\end{figure}

\clearpage
\subsection{S100B vs. Trails B}
We see no significant association between log10(S100B) and Trails B t-scores at either time point.

<<prints100btrailb3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(s100b.trailb.sens.3, 'S100B'),
                       create.pval.table(s100b.trailb.noil6.sens.3, 'S100B')),
                 caption = 'S100B vs 3-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<prints100btrailb12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(s100b.trailb.sens.12, 'S100B'),
                       create.pval.table(s100b.trailb.noil6.sens.12, 'S100B')),
                 caption = 'S100B vs 12-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\begin{figure}[!h]
{\centering\includegraphics{s100b_trailb3_sens_results.pdf}}
\caption{Log10(S100B) vs. Trails B at 3 Months, Adjusting for Log10(IL-6)}\label{fig:s100btrailb3sens}
{\centering\includegraphics{s100b_trailbnoil63_sens_results.pdf}}
\caption{Log10(S100B) vs. Trails B at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:s100btrailbnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{s100b_trailb12_sens_results.pdf}}
\caption{Log10(S100B) vs. Trails B at 12 Months, Adjusting for Log10(IL-6)}\label{fig:s100btrailb12sens}
{\centering\includegraphics{s100b_trailbnoil612_sens_results.pdf}}
\caption{Log10(S100B) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:s100btrailbnoil612sens}\end{figure}

\clearpage
\subsection{UCHL vs. Trails B}
We see no substantial association between log10(UCHL) at baseline and Trails B t-scores at either time point.

<<printuchltrailb3sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(uchl.trailb.sens.3, 'UCHL'),
                       create.pval.table(uchl.trailb.noil6.sens.3, 'UCHL')),
                 caption = 'UCHL vs 3-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@

<<printuchltrailb12sens, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(uchl.trailb.sens.12, 'UCHL'),
                       create.pval.table(uchl.trailb.noil6.sens.12, 'UCHL')),
                 caption = 'UCHL vs 12-Month Trails B',
                 use.colheads = c('P, Adjusting for Log10(IL-6)',
                                  'P, No Adjustment for Log10(IL-6)'))
@
...
\begin{figure}[!h]
{\centering\includegraphics{uchl_trailb3_sens_results.pdf}}
\caption{Log10(UCHL) vs. Trails B at 3 Months, Adjusting for Log10(IL-6)}\label{fig:uchltrailb3sens}
{\centering\includegraphics{uchl_trailbnoil63_sens_results.pdf}}
\caption{Log10(UCHL) vs. Trails B at 3 Months, Not Accounting for Log10(IL-6)}\label{fig:uchltrailbnoil63sens}
\end{figure}

\begin{figure}[!h]
{\centering\includegraphics{uchl_trailb12_sens_results.pdf}}
\caption{Log10(UCHL) vs. Trails B at 12 Months, Adjusting for Log10(IL-6)}\label{fig:uchltrailb12sens}
{\centering\includegraphics{uchl_trailbnoil612_sens_results.pdf}}
\caption{Log10(UCHL) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:uchltrailbnoil612sens}\end{figure}

<<publication_figures_sens>>=
## -- S100B vs RBANS: two panels each for age & inflammation in main paper; ----
## -- four panels for supplement -----------------------------------------------
## -- E-selectin vs RBANS, manuscript version ----------------------------------
## At 3 months, we want 25th and 90th %iles of IL-6; at 12 months, just median (no plots for age)
args.Predict.esel.rbans.sens <- list('x' = list(esel.rbans.sens.3$final.mod,
                                                esel.rbans.sens.3$final.mod,
                                                esel.rbans.sens.12$final.mod),
                                     'il6.log10' = c(il6.1, il6.2, il6.med))

## Create big data frame of all E-selectin vs RBANS model predicted values
Predict.esel.rbans.sens <- pmap(
  args.Predict.esel.rbans.sens,
  .f = Predict,
  esel.log10 = esel.fu.vals
) %>%
  map(.f = as.data.frame)

args.wrangle.esel.rbans.sens <- list('df' = Predict.esel.rbans.sens,
                                     'intvar' = c(rep('il6.log10', 2), 'overall'),
                                     'timept' = c(rep(3, 2), 12))

predvals.esel.rbans.sens <- pmap(
  args.wrangle.esel.rbans.sens,
  .f = wranglePredict
) %>%
  bind_rows() %>%
  mutate(time.f = factor(ifelse(timept == 3, 1, 2),
...
plot.esel.rbans.sens.3 <- ggplot(
  data = subset(predvals.esel.rbans.sens, time.f == '3 Months'),
  aes(x = esel.log10, y = yhat)
) +
  facet_wrap(~ int.f, nrow = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = '#003D79') +
  ...
  scale_y_continuous(name = 'Adjusted RBANS Global Score',
                     limits = c(60, 90), breaks = seq(60, 90, 10)) +
  labs(title = '3-Month Follow-Up', caption = "Adjusted for delirium") +
  theme_minimal() +
  theme(panel.border = element_rect(fill = NA, colour = 'grey80'),
        panel.background = element_rect(fill = 'white', colour = 'grey80'),
        strip.text = element_text(face = 'bold'),
...
plot.esel.rbans.sens.12 <- ggplot(
  data = subset(predvals.esel.rbans.sens, time.f == '12 Months'),
  aes(x = esel.log10, y = yhat)
) +
  facet_wrap(~ int.f, nrow = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = '#003D79') +
  ...
  scale_y_continuous(name = NULL,
                     limits = c(60, 90), breaks = seq(60, 90, 10)) +
  labs(title = '12-Month Follow-Up', caption = "Adjusted for delirium") +
  theme_minimal() +
  theme(panel.border = element_rect(fill = NA, colour = 'grey80'),
        panel.background = element_rect(fill = 'white', colour = 'grey80'),
        strip.text = element_text(face = 'bold'),
...
pdf(file = 'brain_endobbb_eselrbans_ms_sens.pdf', width = 10, height = 4)
multiplot(plot.esel.rbans.sens.3, plot.esel.rbans.sens.12,
          layout = matrix(c(1, 1, 2), nrow = 1))
dev.off()

## -- S100B vs RBANS, manuscript version -----------------------------------------------------------
## For each model, we want to get predicted values for two %iles each of age and IL-6
args.Predict.s100b.rbans.sens <-
  list('x' = list(s100b.rbans.sens.3$final.mod, s100b.rbans.sens.3$final.mod,
                  s100b.rbans.sens.3$final.mod, s100b.rbans.sens.3$final.mod,
                  s100b.rbans.sens.12$final.mod, s100b.rbans.sens.12$final.mod,
                  s100b.rbans.sens.12$final.mod, s100b.rbans.sens.12$final.mod),
       'age.div10' = c(age.1, age.2, age.med, age.med,
                       age.1, age.2, age.med, age.med),
       'il6.log10' = c(il6.med, il6.med, il6.1, il6.2,
...
## Create big data frame of all S100B vs RBANS model predicted values
Predict.s100b.rbans.sens <- pmap(
  args.Predict.s100b.rbans.sens,
  .f = Predict, s100b.log10 = s100b.fu.vals
) %>%
  map(.f = as.data.frame)

args.wrangle.s100b.rbans.sens <-
  list('df' = Predict.s100b.rbans.sens,
       'intvar' = rep(c(rep('age.div10', 2), rep('il6.log10', 2)), 2),
       'timept' = c(rep(3, 4), rep(12, 4)))

predvals.s100b.rbans.sens <- pmap(
  args.wrangle.s100b.rbans.sens, .f = wranglePredict
) %>%
  bind_rows() %>%
  mutate(time.f = factor(ifelse(timept == 3, 1, 2),
                         levels = 1:2, labels = paste(c(3, 12), 'Months')),
...
plot.s100b.rbans.sens.3 <- ggplot(
  data = subset(predvals.s100b.rbans.sens, time.f == '3 Months'),
  aes(x = s100b.log10, y = yhat)
) +
  facet_wrap(~ int.f, nrow = 2) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = '#003D79') +
...
plot.s100b.rbans.sens.12 <- ggplot(
  data = subset(predvals.s100b.rbans.sens, time.f == '12 Months'),
  aes(x = s100b.log10, y = yhat)
) +
  facet_wrap(~ int.f, nrow = 2) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = '#003D79') +
...
pdf(file = 'brain_endobbb_s100brbans_ms_sens.pdf', width = 10, height = 6)
multiplot(plot.s100b.rbans.sens.3, plot.s100b.rbans.sens.12,
          layout = matrix(1:2, nrow = 1))
dev.off()

@

<<publication_forest_sens>>=
## -- Calculate PE, CIs for differences in/IRRs for long-term outcomes for -----
## -- each marker, adjusting to age/IL-6 if needed -----------------------------
...
## -- LTCI outcomes ------------------------------------------------------------
## rms_estimate_with_int doesn't like the $
bdnf.rbans.3.sens.mod <- bdnf.rbans.sens.3$final.mod
bdnf.rbans.12.sens.mod <- bdnf.rbans.sens.12$final.mod
esel.rbans.3.sens.mod <- esel.rbans.sens.3$final.mod
esel.rbans.12.sens.mod <- esel.rbans.sens.12$final.mod
pai1.rbans.3.sens.mod <- pai1.rbans.sens.3$final.mod
pai1.rbans.12.sens.mod <- pai1.rbans.sens.12$final.mod
s100b.rbans.3.sens.mod <- s100b.rbans.sens.3$final.mod
s100b.rbans.12.sens.mod <- s100b.rbans.sens.12$final.mod
uchl.rbans.3.sens.mod <- uchl.rbans.sens.3$final.mod
uchl.rbans.12.sens.mod <- uchl.rbans.sens.12$final.mod

bdnf.trailb.3.sens.mod <- bdnf.trailb.sens.3$final.mod
bdnf.trailb.12.sens.mod <- bdnf.trailb.sens.12$final.mod
esel.trailb.3.sens.mod <- esel.trailb.sens.3$final.mod
esel.trailb.12.sens.mod <- esel.trailb.sens.12$final.mod
pai1.trailb.3.sens.mod <- pai1.trailb.sens.3$final.mod
pai1.trailb.12.sens.mod <- pai1.trailb.sens.12$final.mod
s100b.trailb.3.sens.mod <- s100b.trailb.sens.3$final.mod
s100b.trailb.12.sens.mod <- s100b.trailb.sens.12$final.mod
uchl.trailb.3.sens.mod <- uchl.trailb.sens.3$final.mod
uchl.trailb.12.sens.mod <- uchl.trailb.sens.12$final.mod

## Wrapper function for rms_estimate_with_int which creates argument list
## based on a biomarker prefix (endo.names) and calls pmap_df() to get adjusted
## differences
endo_diffs_sens <- function(endoname, testname = c("rbans", "trailb")){
  ## Things that will be the same for every biomarker:
  intAdjust_vals <- c(age_plotvals, il6_plotvals, age_plotvals, il6_plotvals)
  intVar_vals <- rep(c(rep("age.div10", length(age_plotvals)),
                       rep("il6.log10", length(il6_plotvals))),
  ...
  ## Things that vary based on biomarker:
  rmsObjName_vals <- c(rep(sprintf("%s.%s.3.sens.mod", endoname, testname),
                           length(age_plotvals) + length(il6_plotvals)),
                       rep(sprintf("%s.%s.12.sens.mod", endoname, testname),
                           length(age_plotvals) + length(il6_plotvals)))
  estVals_val <- rep(list(dd$limits[c(1, 3), paste0(endoname, ".log10")]),
                     (length(age_plotvals) + length(il6_plotvals)) * 2)
...
rbans_diffs_sens <- map_df(endo.names, endo_diffs_sens, testname = "rbans") %>%
  endo_plotdata()
trailb_diffs_sens <- map_df(endo.names, endo_diffs_sens, testname = "trailb") %>%
  endo_plotdata()

rbans_diffplot_il6_sens <- plot_ltci_forest(
  rbans_diffs_sens %>% filter(var.int == "il6.log10"),
  testname = "RBANS Global",
  forestlims = c(-9, 4),
  forestbreaks = seq(-9, 3, 3)
)

trailb_diffplot_il6_sens <- plot_ltci_forest(
  trailb_diffs_sens %>% filter(var.int == "il6.log10"),
  testname = "Trails B",
  forestlims = c(-6, 4),
  forestbreaks = seq(-4, 4, 2)
)

rbans_diffplot_age_sens <- plot_ltci_forest(
  rbans_diffs_sens %>% filter(var.int == "age.div10"),
  testname = "RBANS Global",
  forestlims = c(-6, 4),
  forestbreaks = seq(-6, 3, 3)
)

trailb_diffplot_age_sens <- plot_ltci_forest(
  trailb_diffs_sens %>% filter(var.int == "age.div10"),
  testname = "Trails B",
  forestlims = c(-4, 4),
  forestbreaks = seq(-4, 4, 2)
)

pdf(file = "brain_endobbb_rbansdiffs_il6_sens.pdf", width = 10, height = 7)
rbans_diffplot_il6_sens
dev.off()

pdf(file = "brain_endobbb_trailbdiffs_il6_sens.pdf", width = 10, height = 7)
trailb_diffplot_il6_sens
dev.off()

pdf(file = "brain_endobbb_rbansdiffs_age_sens.pdf", width = 10, height = 7)
rbans_diffplot_age_sens
dev.off()

pdf(file = "brain_endobbb_trailbdiffs_age_sens.pdf", width = 10, height = 7)
trailb_diffplot_age_sens
dev.off()

@

<<publication_predprobs_sens>>=
## -- Function to get predicted LTCI scores for a given combo of predictors ----
## Arguments: biomarker; test; time point; interacting var; interacting value
pred_bio_ltci_sens <- function(marker = endo.names,
                               test = c("rbans", "trailb"),
                               timept = c(3, 12),
                               intvar = c("il6.log10", "age.div10"),
                               intval,
  ...
  modname <- paste(c(marker, test, timept, "sens.mod"), collapse = ".")
...
pp_data_sens <- pmap_df(.l = pp_args, .f = pred_bio_ltci_sens)
...
s100b_rbans_il6_ppplot_sens <- plot_ltci_pp(
  pp_data_sens %>% filter(marker == "s100b" & test == "rbans" & intvar == "il6.log10")
)
pdf(file = "brain_endobbb_s100brbans_il6_sens.pdf", width = 10, height = 7)
s100b_rbans_il6_ppplot_sens
dev.off()

s100b_rbans_age_ppplot_sens <- plot_ltci_pp(
  pp_data_sens %>% filter(marker == "s100b" & test == "rbans" & intvar == "age.div10")
)
pdf(file = "brain_endobbb_s100brbans_age_sens.pdf", width = 10, height = 7)
s100b_rbans_age_ppplot_sens
dev.off()

s100b_trailb_il6_ppplot_sens <- plot_ltci_pp(
  pp_data_sens %>% filter(marker == "s100b" & test == "trailb" & intvar == "il6.log10")
)
pdf(file = "brain_endobbb_s100btrailb_il6_sens.pdf", width = 10, height = 7)
s100b_trailb_il6_ppplot_sens
dev.off()

s100b_trailb_age_ppplot_sens <- plot_ltci_pp(
  pp_data_sens %>% filter(marker == "s100b" & test == "trailb" & intvar == "age.div10")
)
pdf(file = "brain_endobbb_s100btrailb_age_sens.pdf", width = 10, height = 7)
s100b_trailb_age_ppplot_sens
dev.off()

esel_rbans_il6_ppplot_sens <- plot_ltci_pp(
  pp_data_sens %>% filter(marker == "esel" & test == "rbans" & intvar == "il6.log10")
)
pdf(file = "brain_endobbb_eselbrbans_il6_sens.pdf", width = 10, height = 7)
esel_rbans_il6_ppplot_sens
dev.off()

esel_rbans_age_ppplot_sens <- plot_ltci_pp(
  pp_data_sens %>% filter(marker == "esel" & test == "rbans" & intvar == "age.div10")
)
pdf(file = "brain_endobbb_eselbrbans_age_sens.pdf", width = 10, height = 7)
esel_rbans_age_ppplot_sens
dev.off()

@

<<supplement_figures_sens>>=
## -- E-selectin vs RBANS, supplement version ------------------------------------------------------
## 3 months only, four panels for IL-6
args.Predict.esel.rbans.sens <- list('x' = list(esel.rbans.sens.3$final.mod),
                                     'il6.log10' = as.numeric(supp.il6.quants))
...
## Create big data frame of all E-selectin vs RBANS model predicted values
Predict.esel.rbans.sens <- pmap(
  args.Predict.esel.rbans.sens, .f = Predict, esel.log10 = esel.fu.vals
) %>%
  map(.f = as.data.frame)

args.wrangle.esel.rbans.sens <-
  list('df' = Predict.esel.rbans.sens, 'intvar' = 'il6.log10', 'timept' = 3)

predvals.esel.rbans.sens <- pmap(args.wrangle.esel.rbans.sens, .f = wranglePredict) %>%
  bind_rows() %>%
  mutate(int.f = ifelse(intvar == 'age.div10', paste('Age* =', round(intval*10)),
                 ifelse(intvar == 'il6.log10',
                        paste('IL-6* =', format(round(intval, 2), nsmall = 2)),
...
suppplot.esel.rbans.3.sens <- ggplot(data = predvals.esel.rbans.sens, aes(x = esel.log10, y = yhat)) +
  facet_wrap(~ int.f, nrow = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = '#003D79') +
  geom_line(colour = '#003D79') +
  scale_x_continuous(name = 'E-Selectin* Plasma Concentration') +
...
pdf(file = 'brain_endobbb_eselrbans3_supp_sens.pdf', width = 10, height = 4)
suppplot.esel.rbans.3.sens
dev.off()

## -- S100B vs RBANS, supplement version: four panels each for age and IL-6 ------------------------
args.Predict.s100b.rbans.sens <- list(
  'x' = list(s100b.rbans.sens.3$final.mod, s100b.rbans.sens.3$final.mod,
             s100b.rbans.sens.3$final.mod, s100b.rbans.sens.3$final.mod,
             s100b.rbans.sens.3$final.mod, s100b.rbans.sens.3$final.mod,
             s100b.rbans.sens.3$final.mod, s100b.rbans.sens.3$final.mod,
             s100b.rbans.sens.12$final.mod, s100b.rbans.sens.12$final.mod,
...
## Create big data frame of all S100B vs RBANS model predicted values
Predict.s100b.rbans.sens <- pmap(
  args.Predict.s100b.rbans.sens, .f = Predict, s100b.log10 = s100b.fu.vals
) %>%
  map(.f = as.data.frame)

args.wrangle.s100b.rbans.sens <- list(
  'df' = Predict.s100b.rbans.sens,
  'intvar' = rep(c(rep('age.div10', 4), rep('il6.log10', 4)), 2),
  'timept' = c(rep(3, 8), rep(12, 8))
)
...
suppplot.s100b.rbans.3.sens <- ggplot(
  data = subset(predvals.s100b.rbans.sens, time.f == '3 Months'),
  aes(x = s100b.log10, y = yhat)
) +
  facet_wrap(~ int.f, nrow = 2, ncol = 4) +
...
suppplot.s100b.rbans.12.sens <- ggplot(
  data = subset(predvals.s100b.rbans.sens, time.f == '12 Months'),
  aes(x = s100b.log10, y = yhat)
) +
  facet_wrap(~ int.f, nrow = 2, ncol = 4) +
...
pdf(file = 'brain_endobbb_s100brbans_supp_sens.pdf', width = 8, height = 8)
multiplot(suppplot.s100b.rbans.3.sens, suppplot.s100b.rbans.12.sens,
          layout = matrix(1:2, nrow = 2))
dev.off()

## -- UCHL vs RBANS, supplement version ----------------------------------------
## 3 months only, no interactions (one panel)
args.Predict.uchl.rbans.sens <- list('x' = list(uchl.rbans.sens.3$final.mod))

## Create data frame of all UCHL vs RBANS model predicted values
Predict.uchl.rbans.sens <- Predict(
  uchl.rbans.sens.3$final.mod, uchl.log10 = uchl.fu.vals
) %>%
  as.data.frame()

suppplot.uchl.rbans.3.sens <- ggplot(
  data = Predict.uchl.rbans.sens, aes(x = uchl.log10, y = yhat)
) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, fill = '#003D79') +
  geom_line(colour = '#003D79') +
  scale_x_continuous(name = 'UCHL* Plasma Concentration') +
...
pdf(file = 'brain_endobbb_uchlrbans3_supp_sens.pdf', width = 6, height = 4)
suppplot.uchl.rbans.3
dev.off()
...
## Sound to let me know it's done
if(require("BRRR")){
