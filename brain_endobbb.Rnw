...
pc.del <- function(mod1, mod2){ pool.compare(mod1, mod2, method = use.method, data = mice.del) }
...
fit.nb.models <- function(use.marker,    ## character string; name of biomarker variable to use
  ...
  p.nonlin <- pc.del(mod.full, mod.lin)$pvalue
  ...
  if(p.nonlin > pcutoff){
    ...
    p.int <- pc.del(mod.lin, mod.noint)$pvalue
    ...
    if(p.int > pcutoff){
      ...
      p.int.age <- pc.del(mod.lin, mod.noage)$pvalue
      ...
      p.int.il6 <- pc.del(mod.lin, mod.noil6)$pvalue
    }
    
    ## 4. Calculate overall p-value for biomarker
    p.marker <- pc.del(mod.final, del.nomark.mod.lin)$pvalue
  } else{
    ...
    ## 3c. Calculate p-value for all interaction terms
    p.int <- pc.del(mod.full, mod.noint)$pvalue
    ...
    if(p.int > 0.2){
      ...
      mod.noage <-
        eval(parse(text = sprintf("with(%s, glm.nb(formula = %s))", mice.obj, marker.noage.formula)))
      p.int.age <- pc.del(mod.full, mod.noage)$pvalue
      
      marker.noil6.formula <- paste(form.split[grep('il6.*:', form.split, invert = TRUE)],
                                    collapse = ' + ')
      mod.noil6 <-
        eval(parse(text = sprintf("with(%s, glm.nb(formula = %s))", mice.obj, marker.noil6.formula)))
      p.int.il6 <- pc.del(mod.full, mod.noil6)$pvalue
    }

    ## 4. Calculate overall p-value for biomarker
    p.marker <- pc.del(mod.final, del.nomark.mod.nonlin)$pvalue
  }
