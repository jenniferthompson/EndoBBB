...
calc.ols.results <- function(olsobj,
                             xvar,
                             intvar = NULL,
                             intvals = NULL){
  if(!is.null(intvar) & is.null(intvals)){
    stop("Interacting variable specified, but no interacting values")
  } else{
    if(is.null(intvar)){
      preddata <- eval(parse(text = sprintf('as.data.frame(Predict(%s, %s = NA))', olsobj, xvar)))
    } else{
      preddata <- eval(parse(text = sprintf('as.data.frame(Predict(%s, %s = NA, %s = c(%s)))',
                                            olsobj, xvar, intvar, paste(intvals, collapse = ', '))))
    }
    return(preddata)
  }
}
