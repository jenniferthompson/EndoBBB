...
calc.ols.results <- function(olsobj,          ## string, name of ols() result
                             ...
                             intvals = NULL){ ## numeric vector, values of intvar
...
plot.ols.results <- function(ols.preddata,     ## data frame of predicted values
                             ...
                             intp = NULL,      ## either p.int.age or p.int.il6 from ols.list
  ...
  if(is.null(intvar)){
    fig <- fig +
      xlab(paste0('Log10(', xstring, ')\n\nP for biomarker, final model: ',
                  formatp(ols.list$p.marker),
                  '\nP for all interactions: ', formatp(ols.list$p.int)))
  } else{
    fig <- fig +
      facet_wrap(~ intvar.f, nrow = 1) +
      xlab(paste0('Log10(', xstring, ')\n\nP for biomarker, final model: ',
                  formatp(ols.list$p.marker),
                  '\nP for all interactions: ', formatp(ols.list$p.int),
                  '\nP for ', intstring, ' interaction: ', formatp(intp))) +
      theme(strip.text = element_text(face = 'bold'))
  }
...
ols.figures <- function(olslist,
  ...
  if(olslist.got$p.int > 0.20){
    ...
    use.intvars <- c('age.div10', 'il6.log10')
    use.intstring <- c('Age / 10', 'Log10(IL-6)')
    use.intp <- c(olslist.got$p.int.age, olslist.got$p.int.il6)
    
    preddata <- calc.ols.results(olsobj = paste0(olslist, '$final.mod'),
                                 xvar = xvar,
                                 intvars = use.intvars,
                                 intvals = list((intvals.age / 10), intvals.il6))
    
    figs <- lapply(1:length(use.intvars), FUN = function(ivar){
      plot.ols.results(ols.preddata = preddata[[ivar]],
                       ols.list = olslist.got,
                       xvar = xvar,
                       intvar = use.intvars[ivar],
                       intstring = use.intstring[ivar],
                       intp = use.intp[ivar],
                       xstring = xstring,
                       ystring = ystring)
    })

    return(list('fig' = figs, 'height' = 8, 'layoutmat' = matrix(1:2, ncol = 1)))
  }
}
