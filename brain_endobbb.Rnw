...
endo.data <- brain.oneobs[brain.oneobs$id %in% marker.data$id, oneobs.vars] %>%
  ...
  mutate(## Create variable for "disabled at baseline" (either ADL or FAQ > 0)
         ...
         cohort.norbans.3 = ifelse(!cog.cohort.3, NA, is.na(rbans.global.score.3)),
         cohort.notrailb.3 = ifelse(!cog.cohort.3, NA, is.na(trail.b.tscore.3)),
         cohort.noadl.3 = ifelse(!func.cohort.3, NA, is.na(adl.totscore.3)),
         cohort.nofaq.3 = ifelse(!func.cohort.3, NA, is.na(faq.totscore.3)),
         cohort.norbans.12 = ifelse(!cog.cohort.12, NA, is.na(rbans.global.score.12)),
         cohort.notrailb.12 = ifelse(!cog.cohort.12, NA, is.na(trail.b.tscore.12)),
         cohort.noadl.12 = ifelse(!func.cohort.12, NA, is.na(adl.totscore.12)),
         cohort.nofaq.12 = ifelse(!func.cohort.12, NA, is.na(faq.totscore.12)),
  ...
  ## and/or had incomplete endothelial markers and/or missing IL-6 (do not want to impute biomarker
  ## values)
  filter((cog.cohort.either | func.cohort.either) & comp.markers & !is.na(il6.day1))

## -- Manually create spline, interaction terms for all biomarkers ---------------------------------
endo.data$bdnf.prime <- rcspline.eval(endo.data$bdnf.log10, nk = 3)
endo.data$esel.prime <- rcspline.eval(endo.data$esel.log10, nk = 3)
endo.data$pai1.prime <- rcspline.eval(endo.data$pai1.log10, nk = 3)
endo.data$s100b.prime <- rcspline.eval(endo.data$s100b.log10, nk = 3)
endo.data$uchl.prime <- rcspline.eval(endo.data$uchl.log10, nk = 3)
endo.data$il6.prime <- rcspline.eval(endo.data$il6.log10, nk = 3)
endo.data$age.prime <- rcspline.eval(endo.data$age.div10, nk = 3)
...
label(endo.data.desc$cohort.norbans.3) <- 'In 3m cognitive cohort, but missing 3m RBANS'
label(endo.data.desc$rbans.global.score.12) <- 'RBANS global score, 12m'
label(endo.data.desc$cohort.norbans.12) <- 'In 12m cognitive cohort, but missing 12m RBANS'
label(endo.data.desc$trail.b.tscore.3) <- 'Trails B T-score, 3m'
label(endo.data.desc$cohort.notrailb.3) <- 'In 3m cognitive cohort, but missing 3m Trails B'
label(endo.data.desc$trail.b.tscore.12) <- 'Trails B T-score, 12m'
label(endo.data.desc$cohort.notrailb.12) <- 'In 12m cognitive cohort, but missing 12m Trails B'
label(endo.data.desc$adl.totscore.3) <- 'ADL score, 3m'
label(endo.data.desc$cohort.noadl.3) <- 'In 3m functional cohort, but missing 3m ADL'
label(endo.data.desc$adl.totscore.12) <- 'ADL score, 12m'
label(endo.data.desc$cohort.noadl.12) <- 'In 12m functional cohort, but missing 12m ADL'
label(endo.data.desc$faq.totscore.3) <- 'FAQ score, 3m'
label(endo.data.desc$cohort.nofaq.3) <- 'In 3m functional cohort, but missing 3m FAQ'
label(endo.data.desc$faq.totscore.12) <- 'FAQ score, 12m'
label(endo.data.desc$cohort.nofaq.12) <- 'In 12m functional cohort, but missing 12m FAQ'

@

\section{Descriptive Statistics}
Tables \ref{table:enrollstat} - \ref{table:fustatfunc} present descriptive statistics for the
...
\item Entire cohort: all patients who had at least one of RBANS global, Trails B, ADL, or FAQ at at
      least one time point \textbf{and} had complete biomarker data (all five endothelial/BBB
      markers present on day 1, as well as IL-6)
\item Cognitive outcomes: all patients who had at least one of RBANS global or Trails B at at least
      one time point and had complete biomarker data
\item Functional outcomes: all patients who had at least one of ADL or FAQ at at least one time
      point and had complete biomarker data
\end{itemize}
...
latex(summaryM(rbans.global.score.3 + cohort.norbans.3 + rbans.global.score.12 + cohort.norbans.12 +
               ...
               ~ cohort,
               data = subset(endo.data.desc, cohort == 'Cognitive Outcomes')),
      file = '',
      where = '!h',
      caption = 'Cognitive Outcomes among Patients with Any Cognitive Data',
      caption.lot = 'Cognitive Outcomes among Patients with Any Cognitive Data',
      label = 'table:fustatcog',
      digits = 2,
      what = '%',
      npct = 'both',
      prmsd = TRUE)

latex(summaryM(adl.totscore.3 + cohort.noadl.3 + adl.totscore.12 + cohort.noadl.12 +
                 faq.totscore.3 + cohort.nofaq.3 + faq.totscore.12 + cohort.nofaq.12 ~ cohort,
               data = subset(endo.data.desc, cohort == 'Functional Outcomes')),
      file = '',
      where = '!h',
      caption = 'Functional Outcomes among Patients with Any Functional Data',
      caption.lot = 'Functional Outcomes among Patients with Any Functional Data',
      label = 'table:fustatfunc',
      digits = 2,
      what = '%',
      npct = 'both',
      prmsd = TRUE)
...
intvals.age <- quantile(endo.data[,'age.enroll'], probs = c(0.25, 0.5, 0.75), na.rm = TRUE)
intvals.il6 <- quantile(endo.data[,'il6.log10'], probs = c(0.25, 0.5, 0.75), na.rm = TRUE)
...
## -- Create aregImpute objects to use for cognitive outcomes --------------------------------------
areg.form <- as.formula("~ age.div10 + charlson.score + I(edu) + I(iqcode.score.e) + stroke.risk +
                           mean.modsofa.icu + icudays.sevseptic.s + del.s.imp + coma.s.imp +
                           il6.log10 + bdnf.log10 + esel.log10 + pai1.log10 + s100b.log10 +
                           uchl.log10 + rbans.global.score.3 + rbans.global.score.12 +
                           trail.b.tscore.3 + trail.b.tscore.12")

set.seed(165)
areg.cog.3 <- aregImpute(areg.form, n.impute = 10, nk = 3, data = subset(endo.data, cog.cohort.3))
set.seed(65)
areg.cog.12 <- aregImpute(areg.form, n.impute = 10, nk = 3, data = subset(endo.data, cog.cohort.12))

## -- Create design matrices for NB models ---------------------------------------------------------
## Default covariate values: 0 (reference level) for factors, median for numeric
covar.adjust <- c('charlson.score', 'iqcode.score.e', 'stroke.risk', 'apache.aps', 'cvsofa.day1',
                  'sevsepsis.day1', 'age.div10', 'il6.log10', 'il6.prime')
default.adj.vals <- lapply(covar.adjust,
                           FUN = function(x){
                             if(is.factor(endo.data[,x])){
                               0
                             } else{
...
fit.nb.models <- function(covars,            ## vector of model covariates;
  ...
  if(is.null(impobj)){
    ...
    modfit.text = "with(impobj, glm.nb(formula = %s))"
  }
...
create.pval.table <- function(fit.reg.obj, marker.string){
  ...
  rownames(p.df) <- c('Global nonlinearity',
                      'Global interactions',
...
latex.pval.table <- function(p.df, use.colheads = c('P-Value')){
  latex(p.df, file = '',
        title = '',
        where = '!h',
        size = 'small',
...
# del.full.formula <- "del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + rcs(il6.log10, 3) + rcs(markervar, 3) + age.div10:rcs(markervar, 3) + rcs(il6.log10, 3):rcs(markervar, 3)"
# 
# ## -- Fit BDNF model by hand -----------------------------------------------------------------------
# ## Original full model: nonlinear terms for BDNF and IL-6, interactions with BDNF * age and IL-6
# bdnf.del.full <- with(mice.del, glm.nb(del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + il6.log10 + bdnf.log10 + bdnf.age + bdnf.il6 + il6.prime + bdnf.prime + bdnfp.age + bdnfp.il6 + bdnf.il6p + bdnfp.il6p))
# 
# # ## Test - make sure coefs are the same
# # bdnf.del.full.test <- with(mice.del, glm.nb(del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + rcs(il6.log10, 3) + rcs(bdnf.log10, 3) + age.div10:rcs(bdnf.log10, 3) + rcs(il6.log10, 3):rcs(bdnf.log10, 3)))
# 
# bdnf.del.lin <- with(mice.del, glm.nb(del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + il6.log10 + bdnf.log10 + bdnf.age + bdnf.il6))
# 
# ## Linear terms necessary?
# pool.compare(bdnf.del.full, bdnf.del.lin, method = 'Wald') ## p = 0.43, so nope
# 
# ## Interaction terms necessary?
# bdnf.del.lin.noint <- with(mice.del, glm.nb(del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + il6.log10 + bdnf.log10))
# 
# pool.compare(bdnf.del.lin, bdnf.del.lin.noint, method = 'Wald') ## p = 0.46, so nope
# 
# ## Final model = linear terms, no interaction
# 
# ## Test with fit.mult.impute
# pai1.rbans.mice.manual <- with(mice.del, lrm(del.s.imp ~ age.div10 + il6.log10 + pai1.log10 + pai1.age + pai1.il6 + il6.prime + pai1.prime + pai1p.age + pai1p.il6 + pai1.il6p + pai1p.il6p))
# 
# pai1.rbans.mice.testnl <- with(mice.del, lrm(del.s.imp ~ age.div10 + il6.log10 + pai1.log10 + pai1.age + pai1.il6))
# 
# pai1.rbans.mice.testint <- with(mice.del, lrm(del.s.imp ~ age.div10 + il6.log10 + pai1.log10))
# 
# pool.compare(pai1.rbans.mice.manual, pai1.rbans.mice.testnl, method = 'Wald')$Dm
# pool.compare(pai1.rbans.mice.testnl, pai1.rbans.mice.testint, method = 'Wald')$pvalue
# 
# pai1.rbans.rms.manual <- fit.mult.impute(del.s.imp ~ age.div10 + il6.log10 + pai1.log10 + pai1.age + pai1.il6 + il6.prime + pai1.prime + pai1p.age + pai1p.il6 + pai1.il6p + pai1p.il6p,
#                                        fitter = lrm,
#                                        xtrans = mice.del,
#                                        data = endo.data)
# 
# anova(pai1.rbans.rms.manual, il6.prime, pai1.prime, pai1p.age, pai1p.il6, pai1.il6p, pai1p.il6p)
# 
# pai1.rbans.rms.lin <- fit.mult.impute(del.s.imp ~ age.div10 + il6.log10 + pai1.log10 + pai1.age + pai1.il6,
#                                        fitter = lrm,
#                                        xtrans = mice.del,
#                                        data = endo.data)
# 
# anova(pai1.rbans.rms.lin, pai1.age, pai1.il6)
# 
# pai1.rbans.rms.rcs <- fit.mult.impute(del.s.imp ~ rcs(pai1.log10, 3)*age.div10 + rcs(pai1.log10, 3)*rcs(il6.log10, 3),
#                                     fitter = lrm,
#                                     xtrans = mice.del,
#                                     data = endo.data)
# 
# pai1.rbans.rms.rcs.lin <- fit.mult.impute(rbans.global.score.3 ~ pai1.log10*age.div10 + pai1.log10*il6.log10,
#                                     fitter = ols,
#                                     xtrans = mice.del,
#                                     data = endo.data)
# 
# pool(pai1.rbans.mice.manual)$qbar
# coef(pai1.rbans.rms.manual)
# coef(pai1.rbans.rms.rcs)
# 
# pool.compare(pai1.rbans.mice.manual, pai1.rbans.mice.testnl, method = 'Wald')$pvalue
# anova(pai1.rbans.rms.rcs)
# 
# pool(pai1.rbans.mice.manual)$t == vcov(pai1.rbans.rms.manual)
# 
...
# # s100b.full.formula <- "del.s.imp ~ rcs(charlson.score, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(apache.aps, 3) + cvsofa.day1 + sevsepsis.day1 + rcs(age.div10, 3) + rcs(il6.log10, 3) + rcs(s100b.log10, 3) + rcs(age.div10, 3):rcs(s100b.log10, 3) + rcs(il6.log10, 3):rcs(s100b.log10, 3)"
# # del.s100b.full <- with(mice.del, glm.nb(formula = as.formula(s100b.full.formula)))
# # 
# # ## Test nonlinear terms
# # s100b.lin.formula <- "del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + il6.log10 + s100b.log10 + age.div10:s100b.log10 + il6.log10:s100b.log10"
# # del.s100b.lin <- with(mice.del, glm.nb(formula = as.formula(s100b.lin.formula)))
# # # lmtest::lrtest(del.s100b.full, del.s100b.lin)
# # pool.compare(del.s100b.full, del.s100b.lin, method = 'Wald', data = mice.del)
# # s100b.del$p.nonlin
# # 
# # ## Nonlinear terms necessary; test interaction terms
# # s100b.noint.formula <- "del.s.imp ~ rcs(charlson.score, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(apache.aps, 3) + cvsofa.day1 + sevsepsis.day1 + rcs(age.div10, 3) + rcs(il6.log10, 3) + rcs(s100b.log10, 3)"
# # del.s100b.noint <- with(mice.del, glm.nb(formula = as.formula(s100b.noint.formula)))
# # pool.compare(del.s100b.full, del.s100b.noint, method = 'Wald', data = mice.del)
# # s100b.del$p.int
# # 
# # ## Interaction nonsignificant; test overall p-value for S100B
# # nomark.formula <- "del.s.imp ~ rcs(charlson.score, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(apache.aps, 3) + cvsofa.day1 + sevsepsis.day1 + rcs(age.div10, 3) + rcs(il6.log10, 3)"
# # del.nomark <- with(mice.del, glm.nb(formula = as.formula(nomark.formula)))
# # pool.compare(del.s100b.noint, del.nomark, method = 'Wald', data = mice.del)
# # s100b.del$p.marker
# 
# ## -- Models without IL-6 --------------------------------------------------------------------------
# ## Formula for full delirium model, subbing "markervar" in place of actual biomarker variable
# del.noil6.formula <- "del.s.imp ~ rcs(charlson.score, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(apache.aps, 3) + cvsofa.day1 + sevsepsis.day1 + rcs(age.div10, 3) + rcs(markervar, 3) + rcs(age.div10, 3):rcs(markervar, 3)"
# 
# fit.del.noil6.models <- function(marker){
#   fit.reg.models(use.marker = marker,
#                  formula.full = del.noil6.formula,
#                  modtype = 'nb',
#                  imp.obj = mice.del,
#                  dataset = endo.data)
# }
# bdnf.del.noil6 <- fit.del.noil6.models('bdnf.log10')
# esel.del.noil6 <- fit.del.noil6.models('esel.log10')
# pai1.del.noil6 <- fit.del.noil6.models('pai1.log10')
# s100b.del.noil6 <- fit.del.noil6.models('s100b.log10')
# uchl.del.noil6 <- fit.del.noil6.models('uchl.log10')
# 
...
Age was divided by ten in order to make the coefficient easier to distinguish from zero; this was
proving to be a computational issue in models using the original scale.
...
%In order to make sure models were stable, all continuous variables were forced to have a linear relationship with Katz ADL scores, including biomarker terms.

We allowed for potential interactions
...
\Sexpr{use.method} method. We used multiple imputation to reduce bias from missing covariates and outcomes.

%Confidence limits for predicted values are calculated using the bootstrap: we ran the final model for each biomarker on \Sexpr{n.bootsets} bootstrapped data sets, calculated the predicted values of ADL score at each unique value of the biomarker for each model fit, and took the 2.5th and 97.5th percentiles of those predicted values as the confidence limits.
...
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = rbans.full.formula.3,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.3,
%'                  cohort.var = 'cog.cohort.3')
%' }
...
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = rbans.full.formula.12,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.12,
%'                  cohort.var = 'cog.cohort.12')
%' }
...
%' ## -- Models without IL-6 --------------------------------------------------------------------------
%' ## Formula for full delirium model, subbing "markervar" in place of actual biomarker variable
%' rbans.righthand.noil6 <- "rcs(charlson.score, 3) + rcs(edu, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(markervar, 3) + rcs(age.div10, 3):rcs(markervar, 3)"
%'
%' rbans.noil6.formula.3 <- paste('rbans.global.score.3', rbans.righthand.noil6, sep = ' ~ ')
%' rbans.noil6.formula.12 <- paste('rbans.global.score.12', rbans.righthand.noil6, sep = ' ~ ')
%'
%' ## 3-month models
%' fit.rbans.noil6.models.3 <- function(marker){
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = rbans.noil6.formula.3,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.3,
%'                  cohort.var = 'cog.cohort.3')
%' }
%' bdnf.rbans.noil6.3 <- fit.rbans.noil6.models.3('bdnf.log10')
%' esel.rbans.noil6.3 <- fit.rbans.noil6.models.3('esel.log10')
%' pai1.rbans.noil6.3 <- fit.rbans.noil6.models.3('pai1.log10')
%' s100b.rbans.noil6.3 <- fit.rbans.noil6.models.3('s100b.log10')
%' uchl.rbans.noil6.3 <- fit.rbans.noil6.models.3('uchl.log10')
%'
...
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = rbans.noil6.formula.12,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.12,
%'                  cohort.var = 'cog.cohort.12')
%' }
%' bdnf.rbans.noil6.12 <- fit.rbans.noil6.models.12('bdnf.log10')
%' esel.rbans.noil6.12 <- fit.rbans.noil6.models.12('esel.log10')
%' pai1.rbans.noil6.12 <- fit.rbans.noil6.models.12('pai1.log10')
%' s100b.rbans.noil6.12 <- fit.rbans.noil6.models.12('s100b.log10')
%' uchl.rbans.noil6.12 <- fit.rbans.noil6.models.12('uchl.log10')
%'
...
%'
%' \subsubsection{BDNF vs. RBANS, 3 Months}
...
%'
%' \clearpage
%' \subsubsection{BDNF vs. RBANS, 12 Months}
...
%' We see a significant association between higher log10(E-selectin) at baseline and RBANS scores 3
%' months after discharge. This association is not modified by either age or log10(IL-6); it remains
%' directionally similar at 12-month followup, but is no longer statistically significant.
%'
%' \subsubsection{E-selectin vs. RBANS, 3 Months}
...
%'
%' \clearpage
%' \subsubsection{E-Selectin vs. RBANS, 12 Months}
...
%' We see no significant association between log10(PAI-1) at baseline and RBANS scores at either 3 or
%' 12 months after discharge.
%'
%' \subsubsection{PAI-1 vs. RBANS, 3 Months}
...
%'
%' \clearpage
%' \subsubsection{PAI-1 vs. RBANS, 12 Months}
...
%' At 3 months, there is a significant relationship between log10(S100B) and RBANS scores; this
%' association is potentially modified by both age and log10(IL-6). At 12 months, the overall
%' association is still present and modified by log10(IL-6), but does not seem to be modified by age.
%'
%' \subsubsection{S100B vs. RBANS, 3 Months}
...
%'
%' \clearpage
%' \subsubsection{S100B vs. RBANS, 12 Months}
...
%' We see a marginally significant, nonlinear relationship between log10(UCHL) and RBANS scores at
%' 3-month followup; this association is not modified by either age or log10(IL-6), and disappears by
%' 12-month followup.
%'
%' \subsubsection{UCHL vs. RBANS, 3 Months}
...
%'
%' \clearpage
%' \subsubsection{UCHL vs. RBANS, 12 Months}
...
%'
%' To examine the association between endothelial/BBB injury at study enrollment and Trails B T-scores at 3- and 12-month followup, we used linear regression, adjusting for the following covariates:
%' \begin{itemize}
%' \item Charlson score
%' \item Years of education
%' \item IQCODE score
...
%' <<trailbmodels>>=
%' ## Formula for full delirium model, subbing "markervar" in place of actual biomarker variable
%' trailb.righthand <- "rcs(charlson.score, 3) + rcs(edu, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(il6.log10, 3) + rcs(markervar, 3) + rcs(age.div10, 3):rcs(markervar, 3) + rcs(il6.log10, 3):rcs(markervar, 3)"
%'
%' trailb.full.formula.3 <- paste('trail.b.tscore.3', trailb.righthand, sep = ' ~ ')
%' trailb.full.formula.12 <- paste('trail.b.tscore.12', trailb.righthand, sep = ' ~ ')
%'
%' ## Set datadist for ols() to work
%' dd <- datadist(endo.data); options(datadist = 'dd')
%'
...
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = trailb.full.formula.3,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.3,
%'                  cohort.var = 'cog.cohort.3')
%' }
%' bdnf.trailb.3 <- fit.trailb.models.3('bdnf.log10')
%' esel.trailb.3 <- fit.trailb.models.3('esel.log10')
%' pai1.trailb.3 <- fit.trailb.models.3('pai1.log10')
%' s100b.trailb.3 <- fit.trailb.models.3('s100b.log10')
%' uchl.trailb.3 <- fit.trailb.models.3('uchl.log10')
%'
%' # ## Check assumptions of final models
...
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = trailb.full.formula.12,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.12,
%'                  cohort.var = 'cog.cohort.12')
%' }
%' bdnf.trailb.12 <- fit.trailb.models.12('bdnf.log10')
%' esel.trailb.12 <- fit.trailb.models.12('esel.log10')
%' pai1.trailb.12 <- fit.trailb.models.12('pai1.log10')
%' s100b.trailb.12 <- fit.trailb.models.12('s100b.log10')
%' uchl.trailb.12 <- fit.trailb.models.12('uchl.log10')
%'
%' # ## Check assumptions of final models
...
%'
%' # ## Test by hand with E-selectin
%' # modvars <- c('trail.b.tscore.12', 'charlson.score', 'edu', 'iqcode.score.e', 'stroke.risk',
%' #              'mean.modsofa.icu', 'icudays.sevseptic.s', 'del.s.imp', 'coma.s.imp', 'age.div10',
%' #              'il6.log10', 'esel.log10')
%' # usedata <- endo.data[rowSums(is.na(endo.data[,modvars])) == 0, modvars]
%' #
%' # ## Fit full model
%' # esel.full.trailb.12 <- ols(trail.b.tscore.12 ~ rcs(charlson.score, 3) + rcs(edu, 3) +
%' #                              iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) +
%' #                              rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) +
%' #                              rcs(age.div10, 3) + rcs(il6.log10, 3) + rcs(esel.log10, 3) +
%' #                              rcs(age.div10, 3):rcs(esel.log10, 3) +
%' #                              rcs(il6.log10, 3):rcs(esel.log10, 3),
%' #                            data = usedata)
%' #
%' # ## Fit model without nonlinear terms
%' # esel.lin.trailb.12 <- ols(trail.b.tscore.12 ~ charlson.score + edu + iqcode.score.e + stroke.risk +
%' #                             mean.modsofa.icu + icudays.sevseptic.s + del.s.imp + coma.s.imp +
%' #                             age.div10 + il6.log10 + esel.log10 + age.div10:esel.log10 +
%' #                             il6.log10:esel.log10,
%' #                           data = usedata)
%' #
%' # lmtest::lrtest(esel.full.trailb.12, esel.lin.trailb.12)
%' # esel.trailb.12$p.nonlin
%' #
%' # ## Nonlinear terms not needed; fit without interaction terms
%' # esel.noint.trailb.12 <- ols(trail.b.tscore.12 ~ charlson.score + edu + iqcode.score.e +
%' #                               stroke.risk + mean.modsofa.icu + icudays.sevseptic.s + del.s.imp +
%' #                               coma.s.imp + age.div10 + il6.log10 + esel.log10,
%' #                             data = usedata)
%' #
%' # lmtest::lrtest(esel.lin.trailb.12, esel.noint.trailb.12)
%' # esel.trailb.12$p.int
%' #
%' # ## Interaction terms not needed; get p-value for biomarker
%' # nomark.full.trailb.12 <- ols(trail.b.tscore.12 ~ charlson.score + edu + iqcode.score.e +
%' #                               stroke.risk + mean.modsofa.icu + icudays.sevseptic.s + del.s.imp +
%' #                               coma.s.imp + age.div10 + il6.log10,
%' #                              data = usedata)
%' # lmtest::lrtest(esel.noint.trailb.12, nomark.full.trailb.12)
%' # esel.trailb.12$p.marker
%'
...
%'
%' ## -- Models without IL-6 --------------------------------------------------------------------------
%' trailb.righthand.noil6 <- "rcs(charlson.score, 3) + rcs(edu, 3) + iqcode.score.e + rcs(stroke.risk, 3) + rcs(mean.modsofa.icu, 3) + rcs(icudays.sevseptic.s, 3) + rcs(del.s.imp, 3) + rcs(coma.s.imp, 3) + rcs(age.div10, 3) + rcs(markervar, 3) + rcs(age.div10, 3):rcs(markervar, 3)"
%'
%' trailb.noil6.formula.3 <- paste('trail.b.tscore.3', trailb.righthand.noil6, sep = ' ~ ')
%' trailb.noil6.formula.12 <- paste('trail.b.tscore.12', trailb.righthand.noil6, sep = ' ~ ')
%'
%' ## 3-month models
%' fit.trailb.noil6.models.3 <- function(marker){
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = trailb.noil6.formula.3,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.3,
%'                  cohort.var = 'cog.cohort.3')
%' }
%' bdnf.trailb.noil6.3 <- fit.trailb.noil6.models.3('bdnf.log10')
%' esel.trailb.noil6.3 <- fit.trailb.noil6.models.3('esel.log10')
%' pai1.trailb.noil6.3 <- fit.trailb.noil6.models.3('pai1.log10')
%' s100b.trailb.noil6.3 <- fit.trailb.noil6.models.3('s100b.log10')
%' uchl.trailb.noil6.3 <- fit.trailb.noil6.models.3('uchl.log10')
%'
...
%'   fit.reg.models(use.marker = marker,
%'                  formula.full = trailb.noil6.formula.12,
%'                  modtype = 'ols',
%'                  imp.obj = areg.cog.12,
%'                  cohort.var = 'cog.cohort.12')
%' }
%' bdnf.trailb.noil6.12 <- fit.trailb.noil6.models.12('bdnf.log10')
%' esel.trailb.noil6.12 <- fit.trailb.noil6.models.12('esel.log10')
%' pai1.trailb.noil6.12 <- fit.trailb.noil6.models.12('pai1.log10')
%' s100b.trailb.noil6.12 <- fit.trailb.noil6.models.12('s100b.log10')
%' uchl.trailb.noil6.12 <- fit.trailb.noil6.models.12('uchl.log10')
%'
%' bdnf.trailb.noil6.12.fig <-
%'   ols.figures.trailb.12(olslist = 'bdnf.trailb.noil6.12', xvar = 'bdnf.log10', xstring = 'BDNF')
%' esel.trailb.noil6.12.fig <-
%'   ols.figures.trailb.12(olslist = 'esel.trailb.noil6.12', xvar = 'esel.log10', xstring = 'E-selectin')
...
%' 3- or 12-month followup.
%'
%' \subsubsection{BDNF vs. Trails B, 3 Months}
%' <<printbdnftrailb3, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = bdnf.trailb.3, marker.string = 'BDNF'),
%'                        create.pval.table(fit.reg.obj = bdnf.trailb.noil6.3, marker.string = 'BDNF')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%'
%' \clearpage
%' \subsubsection{BDNF vs. Trails B, 12 Months}
%' <<printbdnftrailb12, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = bdnf.trailb.12, marker.string = 'BDNF'),
%'                        create.pval.table(fit.reg.obj = bdnf.trailb.noil6.12, marker.string = 'BDNF')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%' \caption{Log10(BDNF) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:bdnftrailbnoil612}
%' \end{figure}
%'
%' \clearpage
%' \subsection{E-Selectin vs. Trails B}
%' We see no significant overall association between log10(E-selectin) at baseline and Trails B
%' T-scores at 3- or 12-month followup.
%'
%' \subsubsection{E-Selectin vs. Trails B, 3 Months}
%' <<printeseltrailb3, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = esel.trailb.3, marker.string = 'E-selectin'),
%'                        create.pval.table(fit.reg.obj = esel.trailb.noil6.3, marker.string = 'E-selectin')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%'
%' \clearpage
%' \subsubsection{E-Selectin vs. Trails B, 12 Months}
%' <<printeseltrailb12, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = esel.trailb.12, marker.string = 'E-selectin'),
%'                        create.pval.table(fit.reg.obj = esel.trailb.noil6.12, marker.string = 'E-selectin')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%' \caption{Log10(E-selectin) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:eseltrailbnoil612}
%' \end{figure}
%'
%'
%' \clearpage
%' \subsection{PAI-1 vs. Trails B}
%' We see no significant association between log10(PAI-1) and Trails B at either time point.
%'
%' \subsubsection{PAI-1 vs. Trails B, 3 Months}
%' <<printpai1trailb3, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = pai1.trailb.3, marker.string = 'PAI-1'),
%'                        create.pval.table(fit.reg.obj = pai1.trailb.noil6.3, marker.string = 'PAI-1')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%'
%' \clearpage
%' \subsubsection{PAI-1 vs. Trails B, 12 Months}
%' <<printpai1trailb12, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = pai1.trailb.12, marker.string = 'PAI-1'),
%'                        create.pval.table(fit.reg.obj = pai1.trailb.noil6.12, marker.string = 'PAI-1')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%' \caption{Log10(PAI-1) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:pai1trailbnoil612}
%' \end{figure}
%'
%'
%' \clearpage
%' \subsection{S100B vs. Trails B}
%' At 3 months, there is not a strong association between log10(S100B) and Trails B scores; there is a
%' potentially weak interaction with age, but any association is not strong enough to make the overall
%' association significant. At 12 months, there is still no significant overall association, but
%' log10(IL-6) is a potential effect modifier.
%'
%' \subsubsection{S100B vs. Trails B, 3 Months}
%' <<prints100btrailb3, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = s100b.trailb.3, marker.string = 'S100B'),
%'                        create.pval.table(fit.reg.obj = s100b.trailb.noil6.3, marker.string = 'S100B')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%'
%' \clearpage
%' \subsubsection{S100B vs. Trails B, 12 Months}
%' <<prints100btrailb12, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = s100b.trailb.12, marker.string = 'S100B'),
%'                        create.pval.table(fit.reg.obj = s100b.trailb.noil6.12, marker.string = 'S100B')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%' \caption{Log10(S100B) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:s100btrailbnoil612}
%' \end{figure}
%'
%' \clearpage
%' \subsection{UCHL vs. Trails B}
%' We see no significant overall association between log10(UCHL) at baseline and Trails B T-scores at
%' 3- or 12-month followup.
%'
%' \subsubsection{UCHL vs. Trails B, 3 Months}
%' <<printuchltrailb3, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = uchl.trailb.3, marker.string = 'UCHL'),
%'                        create.pval.table(fit.reg.obj = uchl.trailb.noil6.3, marker.string = 'UCHL')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%'
%' \clearpage
%' \subsubsection{UCHL vs. Trails B, 12 Months}
%' <<printuchltrailb12, results = 'asis'>>=
%' latex.pval.table(cbind(create.pval.table(fit.reg.obj = uchl.trailb.12, marker.string = 'UCHL'),
%'                        create.pval.table(fit.reg.obj = uchl.trailb.noil6.12, marker.string = 'UCHL')),
%'                  use.colheads = c('P, Adjusting for Log10(IL-6)',
%'                                   'P, No Adjustment for Log10(IL-6)'))
%' @
%'
...
%' \caption{Log10(UCHL) vs. Trails B at 12 Months, Not Accounting for Log10(IL-6)}\label{fig:uchltrailbnoil612}
%' \end{figure}
%'
%'
%' \clearpage
