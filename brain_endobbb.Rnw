...
latex(summaryM(adl.totscore.3 + adl.totscore.12 + faq.totscore.3 + faq.totscore.12 ~ cohort,
...
fit.rbans.noil6.models.3 <- function(marker){
  fit.reg.models(use.marker = marker, formula.full = rbans.noil6.formula.3, modtype = 'ols')
}
...
fit.rbans.noil6.models.12 <- function(marker){
  fit.reg.models(use.marker = marker, formula.full = rbans.noil6.formula.12, modtype = 'ols')
}
...
We see a significant association between log10(E-selectin) at baseline and RBANS scores 3 months
after discharge. This association is somewhat nonlinear, but essentially, higher log10(E-selectin)
is associated with lower RBANS scores up to a certain point. This association is not modified by
either age or log10(IL-6); it remains directionally similar at 12-month followup, but is no longer
...
At 3 months, there is a nearly significant relationship between log10(S100B) and RBANS scores; this
association is potentially modified by both age and log10(IL-6). At 12 months, the overall
association is highly significant and is still modified by log10(IL-6), but is likely not modified
by age.
...
We see a significant, nonlinear relationship between log10(UCHL) and RBANS scores at 3-month
followup; this association is not modified by either age or log10(IL-6), and disappears by
12-month followup.
...
fit.trailb.models.3 <- function(marker){
  fit.reg.models(use.marker = marker, formula.full = trailb.full.formula.3, modtype = 'ols')
}
...
fit.trailb.models.12 <- function(marker){
  fit.reg.models(use.marker = marker, formula.full = trailb.full.formula.12, modtype = 'ols')
}
...
fit.trailb.noil6.models.3 <- function(marker){
  fit.reg.models(use.marker = marker, formula.full = trailb.noil6.formula.3, modtype = 'ols')
}
...
fit.trailb.noil6.models.12 <- function(marker){
  fit.reg.models(use.marker = marker, formula.full = trailb.noil6.formula.12, modtype = 'ols')
}
...
At 3-month followup, there is a possible midl association between log10(PAI-1) and Trails B
T-scores, likely modified by log10(IL-6). However, any association disappears by 12 months.
