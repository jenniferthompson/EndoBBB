...
lrpval <- function(mod1, mod2, pc.method = 'likelihood', mice.obj){
...
fit.reg.models <- function(use.marker,           ## character string; name of biomarker variable
  ...
  ## Initialize all p-values
  p.nonlin <- p.int <- p.marker <- p.il6 <- p.int.age <- p.int.il6 <- NULL
...
create.designmat.nb <- function(nbobj,                ## glm.nb model object, or mice object
  ...
  pred.matrix <- do.call(cbind,
                         lapply(all.term.names, FUN = function(v){
                           if(v %in% colnames(xvals.mat)){
                             xvals.mat[,v]
                           } else{
                             rep(adjust.values[v], nrow(xvals.mat))
                           }
                         }))
  colnames(pred.matrix) <- all.term.names
...
create.pval.table <- function(fit.reg.obj, marker.string){
  p.order <- c('p.nonlin', 'p.int', 'p.marker', 'p.il6', 'p.int.age', 'p.int.il6')

  p.vec <- unlist(lapply(p.order, FUN = function(usep){
    if(is.null(fit.reg.obj[[usep]])){
  ...
  rownames(p.df) <- c('Global nonlinearity',
                      'Global interactions',
                      paste0('Log10(', marker.string, '), all terms'),
                      'Log10(IL-6), all terms',
                      'Age interaction',
...
too little variability to make this sensible.
...
model in order to see if adjusting for systemic inflammation is the reason behind differences from
pilot analyses.
...
# pool.compare(del.s100b.full, del.s100b.lin, method = 'likelihood', data = mice.del)
# s100b.del$p.nonlin
# 
# ## Nonlinear terms not necessary; test interaction terms
# s100b.noint.formula <- "del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + il6.log10 + s100b.log10"
# del.s100b.noint <- with(mice.del, glm.nb(formula = as.formula(s100b.noint.formula)))
# pool.compare(del.s100b.lin, del.s100b.noint, method = 'likelihood', data = mice.del)
# s100b.del$p.int
# 
# ## Interaction nonsignificant; test overall p-value for S100B
# nomark.formula <- "del.s.imp ~ charlson.score + iqcode.score.e + stroke.risk + apache.aps + cvsofa.day1 + sevsepsis.day1 + age.div10 + il6.log10"
# del.nomark <- with(mice.del, glm.nb(formula = as.formula(nomark.formula)))
# pool.compare(del.s100b.noint, del.nomark, method = 'likelihood', data = mice.del)
# s100b.del$p.marker
...
bdnf.del.noil6.fig <- nb.figures.del(nbobj = bdnf.del.noil6, xvar = 'bdnf.log10', xstring = 'BDNF')
esel.del.noil6.fig <-
  nb.figures.del(nbobj = esel.del.noil6, xvar = 'esel.log10', xstring = 'E-Selectin')
pai1.del.noil6.fig <- nb.figures.del(nbobj = pai1.del.noil6, xvar = 'pai1.log10', xstring = 'PAI-1')
s100b.del.noil6.fig <-
  nb.figures.del(nbobj = s100b.del.noil6, xvar = 's100b.log10', xstring = 'S100B')
uchl.del.noil6.fig <- nb.figures.del(nbobj = uchl.del.noil6, xvar = 'uchl.log10', xstring = 'UCHL')
...
pdf(file = 'bdnf_delnoil6_results.pdf', width = 8, height = bdnf.del.noil6.fig$height)
multiplot(plotlist = bdnf.del.noil6.fig$fig, layout = bdnf.del.noil6.fig$layoutmat)
dev.off()
...
pdf(file = 'esel_delnoil6_results.pdf', width = 8, height = esel.del.noil6.fig$height)
multiplot(plotlist = esel.del.noil6.fig$fig, layout = esel.del.noil6.fig$layoutmat)
dev.off()
...
pdf(file = 'pai1_delnoil6_results.pdf', width = 8, height = pai1.del.noil6.fig$height)
multiplot(plotlist = pai1.del.noil6.fig$fig, layout = pai1.del.noil6.fig$layoutmat)
dev.off()
...
pdf(file = 's100b_delnoil6_results.pdf', width = 8, height = s100b.del.noil6.fig$height)
multiplot(plotlist = s100b.del.noil6.fig$fig, layout = s100b.del.noil6.fig$layoutmat)
dev.off()
...
pdf(file = 'uchl_delnoil6_results.pdf', width = 8, height = uchl.del.noil6.fig$height)
multiplot(plotlist = uchl.del.noil6.fig$fig, layout = uchl.del.noil6.fig$layoutmat)
dev.off()
...
latex.pval.table(cbind(create.pval.table(bdnf.del, 'BDNF'),
                       create.pval.table(bdnf.del.noil6, 'BDNF')),
...
latex.pval.table(cbind(create.pval.table(esel.del, 'E-selectin'),
                       create.pval.table(esel.del.noil6, 'E-selectin')),
                       use.colheads = c('P, Adjusting for Log10(IL-6)',
                                        'P, No Adjustment for Log10(IL-6)'))
@
...
We see a nearly significant association between log(PAI-1) at baseline and delirium duration while in the hospital when adjusting for systemic inflammation; this association is modified by age. When
not accounting for systemic inflammation, however, we do not see a meaningful association between
log10(PAI-1) and delirium.

<<printpai1del, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(pai1.del, 'PAI-1'),
                       create.pval.table(pai1.del.noil6, 'PAI-1')),
                       use.colheads = c('P, Adjusting for Log10(IL-6)',
                                        'P, No Adjustment for Log10(IL-6)'))
@
...
hospital when accounting for systemic inflammation. When not adjusting for systemic inflammation,
however, we do see a significant association, likely modified by age at enrollment.

<<prints100bdel, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(s100b.del, 'S100B'),
                       create.pval.table(s100b.del.noil6, 'S100B')),
                       use.colheads = c('P, Adjusting for Log10(IL-6)',
                                        'P, No Adjustment for Log10(IL-6)'))
@
...
UCHL at baseline does show a significant or nearly significant association with days of delirium,
whether adjusting for systemic inflammation or not. When we account for systemic inflammation, the
association is not modified by age, but when we ignore systemic inflammation, the association
becomes stronger (and more nonlinear) with age.

<<printuchldel, results = 'asis'>>=
latex.pval.table(cbind(create.pval.table(uchl.del, 'UCHL'),
                       create.pval.table(uchl.del.noil6, 'UCHL')),
                       use.colheads = c('P, Adjusting for Log10(IL-6)',
                                        'P, No Adjustment for Log10(IL-6)'))
@
